...
TEST_CASE("viewport surfaces keep dimensions and render options independent")
{
    ...
    REQUIRE(host->execute(arc::editor::host_viewport_resize_command{
                              .viewport_id = "viewport-1", .width = 1024, .height = 768})
                .succeeded);
    ...
    const auto missing =
        host->execute(arc::editor::host_viewport_resize_command{.viewport_id = "missing", .width = 10, .height = 10});
...
TEST_CASE("viewport surfaces keep local frame indices independent")
{
    ...
    REQUIRE(host->execute(arc::editor::host_viewport_create_command{
                              .viewport_id = "shader-preview", .width = 256, .height = 256})
                .succeeded);

    const auto scene_frame =
        host->request_viewport({.viewport_id = "viewport-1", .frame_index = 7, .width = 640, .height = 480});
    const auto preview_frame =
        host->request_viewport({.viewport_id = "shader-preview", .frame_index = 3, .width = 256, .height = 256});
...
TEST_CASE("detaching one viewport surface does not detach another")
{
    ...
    REQUIRE(host->execute(arc::editor::host_viewport_create_command{
                              .viewport_id = "material-preview", .width = 300, .height = 300})
                .succeeded);
...
TEST_CASE("material preview mesh and animation options stay on the existing viewport surface")
{
    auto renderer = std::make_unique<arc::render::renderer>();
    arc::editor::arc_host_manager manager;
    auto host = manager.acquire(std::move(renderer));
    ...
    CHECK(preview.at("viewportId") == viewport_id);
    CHECK(preview.at("assetPreviewMesh") == "cube");
    CHECK(preview.at("assetPreviewAutoRotate") == false);
    CHECK(preview.at("assetPreviewReady") == false);

    const auto invalid = host->execute(arc::editor::host_viewport_set_render_options_command{
        .viewport_id = viewport_id, .material_preview_mesh = "teapot"});
    CHECK_FALSE(invalid.succeeded);
    CHECK(invalid.error == "Unsupported material preview mesh: teapot");
}
