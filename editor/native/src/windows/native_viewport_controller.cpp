...
class win32_native_viewport_controller;
...
class win32_native_viewport_controller final : public arc::editor::native_viewport_controller
{
public:
    win32_native_viewport_controller(std::shared_ptr<arc::editor::arc_host> host, std::mutex& host_mutex,
    ...
    ~win32_native_viewport_controller() override
    {
        stop();
    }
    ...
    bool create_shared(std::string viewport_id, std::uint64_t consumer_process_id, std::uint32_t width,
                       std::uint32_t height, std::string& error) override
    {
    ...
    void release_frame(std::string viewport_id, std::uint64_t generation, std::uint64_t frame_id,
                       std::string consumer_handle) override
    {
    ...
    void set_visible(std::string_view viewport_id, bool visible) override
    {
        std::lock_guard lock(bounds_mutex_);
        if (shared_texture_)
        {
    ...
    void pointer(const arc::editor::host_viewport_pointer_command& pointer) override
    {
        std::lock_guard lock(bounds_mutex_);
        if (shared_texture_)
        {
    ...
    void key(const arc::editor::host_viewport_key_command& key) override
    {
        std::lock_guard lock(bounds_mutex_);
        if (shared_texture_)
        {
...
public:
    void detach(std::string_view viewport_id) override
    {
        std::lock_guard lock(bounds_mutex_);
        if (shared_texture_)
        {
...
LRESULT CALLBACK native_viewport_wnd_proc(HWND window, UINT message, WPARAM wparam, LPARAM lparam)
{
    ...
    if (auto* controller =
            reinterpret_cast<win32_native_viewport_controller*>(GetWindowLongPtrW(window, GWLP_USERDATA)))
...
{
    return std::make_unique<win32_native_viewport_controller>(std::move(host), host_mutex, output_mutex, jobs);
}
