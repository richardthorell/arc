...
void append_overlay_line(render::debug_overlay_stream& stream, const math::vector3f& start, const math::vector3f& end,
...
void append_scene_component_icons(render::debug_overlay_stream& stream, const ecs::world& registry,
                                  ...
                                  const scene::camera_component& view_camera,
                                  const scene::transform_component& view_transform, const editor_gizmo_context& context)
{
                                ...
                                const math::vector3f& grid_color)
{
    ...
    const auto with_alpha = [&](const math::vector3f& color, float multiplier, float alpha)
    {
        return math::vector4f{
            std::clamp(color[0] * multiplier, 0.0f, 1.0f), std::clamp(color[1] * multiplier, 0.0f, 1.0f),
            std::clamp(color[2] * multiplier, 0.0f, 1.0f), std::clamp(alpha * opacity_scale, 0.0f, 1.0f)};
    };
    ...
    const auto faded_minor =
        math::vector4f{minor_color[0], minor_color[1], minor_color[2], minor_color[3] * (1.0f - level_blend)};
    const auto lerp_color = [](const math::vector4f& first, const math::vector4f& second, float amount)
    {
        return math::vector4f{first[0] + (second[0] - first[0]) * amount, first[1] + (second[1] - first[1]) * amount,
                              first[2] + (second[2] - first[2]) * amount, first[3] + (second[3] - first[3]) * amount};
    };
    const auto transitioning_major = lerp_color(major_color, minor_color, level_blend);

    constexpr std::array<math::vector3f, 3> grid_axis_colors{math::vector3f{0.7882353f, 0.3372549f, 0.2980392f},
                                                             math::vector3f{0.4117647f, 0.7098039f, 0.3568627f},
                                                             math::vector3f{0.2980392f, 0.4980392f, 0.8196079f}};
    const auto axis_color = [&](std::size_t axis) { return with_alpha(grid_axis_colors[axis], 1.0f, axis_alpha); };
    ...
    const auto append_line = [&](const math::vector3f& first, const math::vector3f& second, const math::vector4f& color)
    {
