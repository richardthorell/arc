...
type UiTreeRowProps = UiTreeRowCommonProps &
  (({ as?: 'button' } & ButtonHTMLAttributes<HTMLButtonElement>) | ({ as: 'div' } & HTMLAttributes<HTMLDivElement>));

type TreeRowStyle = CSSProperties & {
  '--ui-tree-depth'?: string;
};

export function UiTreeRow({
  as = 'button',
  children,
  className,
  depth = 0,
  ...
  if (as === 'div') {
    return (
      <div className={classes} style={rowStyle} {...(props as HTMLAttributes<HTMLDivElement>)}>
        {contents}
      </div>
    );
  }
