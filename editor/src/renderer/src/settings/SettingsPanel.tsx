...
export function SettingsPanel({ onResetLayout }: { onResetLayout: () => void }) {
  ...
  return (
    <section className="production-tool-panel settings-production-panel">
      <header className="tool-panel-toolbar">
        <Settings size={15} />
        <strong>Settings</strong>
        <select value={scope} onChange={(event) => setScope(event.target.value as 'user' | 'project')}>
