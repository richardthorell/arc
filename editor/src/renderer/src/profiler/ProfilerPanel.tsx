...
export function ProfilerPanel({ samples }: { samples: ProfilerSnapshot[] }) {
  ...
  const groups = latest.memory.groups.filter(
    (group) =>
    ...
    <section className="profiler-panel">
      <div className="profiler-summary">
        <article>
          ...
          <small>
            {memoryPercent.toFixed(1)}% of {formatBytes(budget)}
          </small>
        </article>
        <article>
        ...
        <article>
          ...
          <small>
            {latest.scheduler.cancelled} cancelled · {latest.scheduler.failed} failed
          </small>
        </article>
      ...
      <div className="profiler-main">
        <div className="profiler-memory-column">
          <header>
            ...
            <span>5 minute rolling window</span>
          </header>
          <svg
            className="profiler-chart"
            viewBox="0 0 100 40"
          ...
          <div className="profiler-domain-list">
            {latest.memory.domains
              .filter((item) => item.bytes > 0 || item.peakBytes > 0)
              .map((item) => {
                ...
                return (
                  <button
                    type="button"
                    key={item.domain}
                    className={item.pressure ? 'pressure' : ''}
        ...
        <div className="profiler-detail-column">
          <div className="profiler-filters">
            <select value={domain} onChange={(event) => setDomain(event.target.value)}>
              ...
              {unique(latest.memory.groups.map((group) => group.domain)).map((value) => (
                <option key={value}>{value}</option>
              ))}
            </select>
            <select value={tag} onChange={(event) => setTag(event.target.value)}>
              <option value="all">All tags</option>
              {unique(latest.memory.groups.map((group) => group.tag)).map((value) => (
                <option key={value}>{value}</option>
              ))}
            </select>
            ...
            <select value={stack} onChange={(event) => setStack(event.target.value)}>
              ...
              {unique(latest.memory.groups.map((group) => group.stackId)).map((value) => (
                <option key={value} value={String(value)}>
                  {value ? `0x${value.toString(16)}` : 'Unsampled'}
                </option>
              ))}
            </select>
          </div>
          ...
          <div className="profiler-tables">
            <div>
              <header>
                <strong>Allocation Groups</strong>
              ...
              <div className="profiler-table" data-testid="profiler-allocation-groups">
                {groups.slice(0, 100).map((group, index) => (
                  <div key={`${group.domain}-${group.tag}-${group.stackId}-${index}`}>
                    <span>
                      {group.tag}
                      <small>
                        {group.domain} · {group.allocationCount} allocs
                      </small>
                    </span>
            ...
            <div>
              <header>
                <strong>Recent Task Scopes</strong>
              ...
              <div className="profiler-table">
                {jobs
                  ...
                  .reverse()
                  .map((job) => (
                    <div key={job.sequence}>
                      <span>
                        {job.name}
                        <small>
                          {job.affinity} · {job.priority} · {job.status}
                        </small>
                      </span>
