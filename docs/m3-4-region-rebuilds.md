...
Interactive sculpting now uses regular-grid ray traversal, never waits for an asset load on pointer down, coalesces compatibility preview dirtiness to one viewport publication, and recompiles only render regions overlapping the dirty sample rectangle. The M3.4 asset-owned rebuild remains the authoritative post-stroke result.
