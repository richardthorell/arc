...
void main()
{
    ...
    vec2 source_sample = mix(patch_samples.xy, patch_samples.zw, uv);
    ...
    out_normal = normalize(transpose(inverse(mat3(constants.model))) * local_normal);
    out_world_position = world_position.xyz;
    out_texcoord = source_sample / source_quads;
    out_tangent = vec4(normalize(mat3(constants.model) * vec3(1.0, 0.0, 0.0)), 1.0);
    out_clip_position = constants.model_view_projection * vec4(local_position, 1.0);
    mat4 previous_mvp = mat4(constants.light_direction_intensity,
    ...
    out_previous_clip_position = previous_mvp * vec4(local_position, 1.0);
