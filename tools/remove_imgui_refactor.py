...
old_config = """#if ARC_RENDER_VULKAN_ENABLE_IMGUI
...
#endif"""
new_config = """        if (native_swapchain_initialized_ && previous_scale != config.render_scale && output_viewport_width_ > 0 &&
            output_viewport_height_ > 0)
        {
            ensure_viewport(scaled_dimension(output_viewport_width_), scaled_dimension(output_viewport_height_));
        }"""
replace_once(old_config, new_config, "configure viewport resize")

resize_start = text.index("    void resize_viewport(std::uint32_t width, std::uint32_t height) override")
profile_start = text.index("    render_backend_frame_profile last_frame_profile() const override", resize_start)
text = text[:resize_start] + """    void resize_viewport(std::uint32_t width, std::uint32_t height) override
    {
    ...
    render_viewport_texture viewport_texture() const noexcept override
    {
        ...
        // ImGui texture handle is intentionally no longer exposed.
        return {};
    }

""" + text[profile_start:]

legacy_start = text.index("    bool initialize_imgui(")
surface_start = text.index("    surface_frame_result present_surface_frame", legacy_start)
text = text[:legacy_start] + text[surface_start:]

native_start = text.index("    bool render_native_viewport_frame(")
shutdown_start = text.index("    void shutdown_imgui() noexcept", native_start)
native = """    bool render_native_viewport_frame(std::uint32_t width, std::uint32_t height, std::string& message)
    {
        message.clear();
        if (device_lost_)
        {
...
"""
text = text[:native_start] + native + text[shutdown_start:]

shutdown_start = text.index("    void shutdown_imgui() noexcept")
private_start = text.index("private:", shutdown_start)
shutdown = """    void shutdown_surface_presenter() noexcept
    {
...
"""
text = text[:shutdown_start] + shutdown + text[private_start:]

scaled_guard = """#if ARC_RENDER_VULKAN_ENABLE_IMGUI
...
#endif"""
scaled_new = """    std::uint32_t scaled_dimension(std::uint32_t value) const noexcept
    {
        return std::max(
            1u, static_cast<std::uint32_t>(std::round(static_cast<float>(value) * resolved_config_.render_scale)));
    }
...
replace_once(scaled_guard, scaled_new, "frame wait helpers")

text, count = re.subn(r"\n    struct vulkan_swapchain_state\n    \{.*?\n    \};\n", "\n", text, count=1, flags=re.S)
if count != 1:
    raise RuntimeError(f"obsolete swapchain state: expected one match, found {count}")
...
clustered = """#if ARC_RENDER_VULKAN_ENABLE_IMGUI
...
""",
    "",
    "ImGui viewport texture registration",
)

viewport_descriptor_block = """        if (viewport_descriptor_ != VK_NULL_HANDLE)
        {
            ImGui_ImplVulkan_RemoveTexture(viewport_descriptor_);
            viewport_descriptor_ = VK_NULL_HANDLE;
        }
"""
replace_once(viewport_descriptor_block, "", "ImGui viewport texture removal")
text = text.replace("    vulkan_swapchain_state swapchain_state_{};\n", "")

member_start = text.index("#if ARC_RENDER_VULKAN_ENABLE_IMGUI\n    ImGui_ImplVulkanH_Window window_{};")
member_end = text.index("#endif", member_start) + len("#endif")
member_new = """    detail::vulkan_swapchain swapchain_{};
...
text = text[:member_start] + member_new + text[member_end:]

forbidden = ["ImGui", "imgui", "ARC_RENDER_VULKAN_ENABLE_IMGUI", "window_", "viewport_descriptor_"]
remaining = [token for token in forbidden if token in text]
if remaining:
    raise RuntimeError(f"legacy presentation references remain: {remaining}")
