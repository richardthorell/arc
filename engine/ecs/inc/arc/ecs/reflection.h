...
inline constexpr runtime_component_index invalid_runtime_component_index = static_cast<runtime_component_index>(-1);
...
{
    return static_cast<reflected_field_flags>(static_cast<std::uint16_t>(lhs) | static_cast<std::uint16_t>(rhs));
}
...
struct component_field_descriptor
{
    ...
    reflected_field_flags flags{reflected_field_flags::serialized | reflected_field_flags::editable |
...
template <class T> constexpr const component_descriptor& component_metadata() noexcept;
...
public:
    bool register_component(const component_descriptor& descriptor)
    {
        if (frozen_ || !descriptor.id.valid() ||
            std::any_of(descriptors_.begin(), descriptors_.end(),
                        [&](const component_descriptor* existing) { return existing->id == descriptor.id; }))
            return false;
        descriptors_.push_back(&descriptor);
        return true;
    }

    template <class T> bool register_component()
    {
    ...
    bool freeze()
    {
        ...
        if (descriptors_.size() >= invalid_runtime_component_index) return false;
    ...
    {
        if (!frozen_) return invalid_runtime_component_index;
...
template <class T> constexpr std::string_view compiler_type_name() noexcept
{
#if defined(_MSC_VER)
...
{
    return {fnv1a(name, 14695981039346656037ull), fnv1a(name, 1099511628211ull)};
}
...
inline std::size_t field_index(const component_descriptor& descriptor, component_field_id id) noexcept
{
    for (std::size_t index = 0; index < descriptor.fields.size(); ++index)
        if (descriptor.fields[index].id == id) return index;
