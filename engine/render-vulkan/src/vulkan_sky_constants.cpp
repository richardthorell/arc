...
std::uint32_t quantize_unsigned(float value, float maximum, std::uint32_t mask) noexcept
{
    ...
    return static_cast<std::uint32_t>(std::round(std::clamp(value / maximum, 0.0f, 1.0f) * static_cast<float>(mask)));
}
...
void pack_cloud_layer(const cloud_layer_data& layer, bool clouds_enabled, float time_seconds,
                      float* destination) noexcept
{
    if (!clouds_enabled || !layer.enabled) return;
    ...
    if (math::length_squared(wind) > minimum_projection_scale) wind = math::normalize(wind);
...
std::uint32_t pack_moon_settings_bits(float phase, float intensity, float angular_radius_degrees, bool enabled) noexcept
{
    return quantize_unsigned(phase, 1.0f, 0x3ffu) | (quantize_unsigned(intensity, 8.0f, 0x3ffu) << 10u) |
           (quantize_unsigned(angular_radius_degrees, 5.0f, 0x3ffu) << 20u) | (enabled ? 1u << 30u : 0u);
}

std::uint32_t pack_sky_settings_bits(sky_source_mode source, float star_density, float star_intensity) noexcept
{
    return static_cast<std::uint32_t>(source) | (quantize_unsigned(star_density, 1.0f, 0x3ffu) << 2u) |
           (quantize_unsigned(star_intensity, 16.0f, 0xfffu) << 12u);
}

sky_push_constants build_sky_push_constants(const world_environment_data& environment, const render_camera& camera,
                                            std::uint32_t viewport_width, std::uint32_t viewport_height,
                                            bool include_cirrus, math::vector3f sun_direction_override) noexcept
{
    ...
    if (math::length_squared(sun_direction) < minimum_projection_scale) sun_direction = default_sun_direction;
    ...
    constants.camera_up_aspect[3] =
        viewport_height == 0 ? 1.0f : static_cast<float>(viewport_width) / static_cast<float>(viewport_height);
    ...
    constants.moon_direction_phase[3] = packed_float(
        pack_moon_settings_bits(environment.celestial.moon_phase, environment.celestial.moon_intensity,
                                environment.celestial.moon_angular_radius_degrees, environment.celestial.moon_enabled));

    const auto sky_color =
        environment.source == sky_source_mode::solid_color ? environment.solid_color : environment.atmosphere.tint;
    for (std::uint32_t channel = 0; channel < 3; ++channel)
    ...
    constants.sky_color_source[3] = packed_float(
        pack_sky_settings_bits(environment.source, star_density, environment.celestial.star_intensity * star_twinkle));
    ...
    constants.atmosphere[3] =
        environment.radiance_intensity * std::max(environment.atmosphere.exposure, minimum_environment_exposure);

    pack_cloud_layer(environment.clouds.cumulus, environment.clouds.enabled, environment.celestial.time_seconds,
                     constants.cumulus);
    if (include_cirrus)
    {
        pack_cloud_layer(environment.clouds.cirrus, environment.clouds.enabled, environment.celestial.time_seconds,
                         constants.cirrus);
    }
    return constants;
}
