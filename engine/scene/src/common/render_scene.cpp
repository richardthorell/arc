...
geometric::box3f transform_bounds(const geometric::box3f& local_bounds, const math::matrix4f& world)
{
    ...
    {
        return math::vector3f{(index & 1u) != 0u ? local_bounds.max[0] : local_bounds.min[0],
    ...
    geometric::box3f result{geometric::point3f{transformed}, geometric::point3f{transformed}};
...
geometric::box3f world_bounds_for(const ecs::world& scene, entity value, const transform_component& transform)
{
    ...
    if (bounds)
        return bounds->dirty
                   ? transform_bounds(bounds->local_bounds, transform.dirty ? local_matrix(transform) : transform.world)
                   : bounds->world_bounds;
    return geometric::box3f{geometric::point3f{transform.position - math::vector3f{0.5f, 0.5f, 0.5f}},
                      ...
                      float distance_field_resolution_bias = 0.0f, bool visible_in_hardware_tracing = true)
{
    ...
    packet.items.push_back({.mesh = mesh,
                            .material = material,
                            .model = world,
...
{
    transparent = false;
    if (const auto* terrain = scene.try_get<terrain_component>(value)) return visibility.terrain && terrain->enabled;
...
void apply_lod(const ecs::world& scene, entity value, render::mesh_handle& mesh, render::material_handle& material)
{
    const auto* lod = scene.try_get<lod_component>(value);
    if (!lod || !lod->enabled || lod->levels.empty()) return;
...
render::world_environment_data to_render_environment(const ecs::world& scene, entity value,
                                                     ...
                                                     const scene_render_visibility& visibility)
{
    ...
    const auto moon_direction = math::normalize(
        math::vector3f{-sun_direction[0] + moon_orbit_x_offset * std::sin(phase * math::tau<float>), -sun_direction[1],
    ...
    render::world_environment_data output{
        ...
        .atmosphere = {.enabled = atmosphere.enabled,
                       ...
                       .label = label},
        .celestial = {.enabled = celestial.enabled,
                      .sun_direction = sun_direction,
                      .moon_direction = moon_direction,
                      .sun_angular_radius_degrees = std::max(minimum_sun_angular_radius_degrees,
        ...
        .fog = {.enabled = world.enabled && visibility.fog && fog.enabled,
                .color = fog.color,
                .density = fog.density,
...
render_scene_result render_scene(ecs::world& scene, render::renderer& renderer, std::uint32_t viewport_width,
                                 std::uint32_t viewport_height, render::render_mode mode,
                                 ...
                                 entity preferred_camera, terrain_render_proxy_cache* terrain_proxies)
{
    ...
    scene.view<world_environment_component>().each(
        [&](entity value, const world_environment_component&)
        {
            ...
            if (world_packet.environment.fog.enabled) ++result.fog_count;
        });
    ...
    scene.view<transform_component, terrain_component>().each(
        [&](entity value, const transform_component& transform, const terrain_component& terrain)
        {
            ...
            const geometric::box3f local_bounds{
                geometric::point3f{static_cast<float>(proxy->local_bounds.min_x),
    ...
    scene.view<transform_component, vegetation_component>().each(
        ...
        {
            if (!environment_visibility.vegetation || !entity_is_active(scene, value) || !vegetation.enabled) return;
    ...
    scene.view<transform_component, decal_component>().each(
        ...
        {
            if (!environment_visibility.decals || !entity_is_active(scene, value) || !decal.enabled) return;
            const math::matrix4f world = transform.dirty ? local_matrix(transform) : transform.world;
            world_packet.decals.push_back({.object_id = render::make_render_object_id(value.index, value.generation),
                                           .model = world,
                                           .world_bounds = world_bounds_for(scene, value, transform),
                                           .color = decal.color,
    ...
    scene.view<transform_component, directional_light_component>().each(
        ...
        {
            if (!entity_is_active(scene, value) || !light.enabled) return;
    ...
    const auto lighting = render::pack_scene_lighting(world_packet.directional_lights, world_packet.point_lights,
                                                      world_packet.spot_lights, nullptr, render::max_point_lights,
                                                      render::max_spot_lights, world_packet.area_lights);
