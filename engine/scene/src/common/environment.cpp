...
double julian_day(std::int32_t year, std::int32_t month, std::int32_t day, float utc_hours) noexcept
{
    if (month <= 2)
    {
        --year;
    ...
    return std::floor(365.25 * (year + 4716)) + std::floor(30.6001 * (month + 1)) + day + correction - 1524.5 +
           utc_hours / 24.0;
}
...
math::quatf rotation_from_forward(const math::vector3f& direction) noexcept
{
    ...
    if (dot < -0.9999f) return math::quatf{0.0f, 1.0f, 0.0f, 0.0f};
...
void use_system_time(celestial_sky_component& celestial)
{
    ...
    celestial.local_time_hours = static_cast<float>(utc.tm_hour) + static_cast<float>(utc.tm_min) / minutes_per_hour +
...
bool finite(const math::vector2f& value) noexcept
{
...
void validate_cloud_layer(const cloud_layer_settings& layer, const char* name, environment_validation_result& result)
{
    ...
    if (!finite(layer.detail) || layer.detail < 0.0f || layer.detail > 1.0f || !finite(layer.softness) ||
        layer.softness < 0.0f || layer.softness > 1.0f || !finite(layer.lighting_strength) ||
        layer.lighting_strength < 0.0f || !finite(layer.silver_lining) || layer.silver_lining < 0.0f ||
        layer.silver_lining > 1.0f)
        result.errors.emplace_back(std::string(name) +
                                   " detail, softness, lighting, and silver lining are out of range");
                                        ...
                                        float utc_offset_hours) noexcept
{
    ...
    const float gamma =
        math::tau<float> / year_days *
        (static_cast<float>(day_of_year(year, month, day) - 1) + (local_time_hours - 12.0f) / hours_per_day);
    const float equation_of_time = 229.18f * (0.000075f + 0.001868f * std::cos(gamma) - 0.032077f * std::sin(gamma) -
                                              0.014615f * std::cos(2.0f * gamma) - 0.040849f * std::sin(2.0f * gamma));
    const float declination = 0.006918f - 0.399912f * std::cos(gamma) + 0.070257f * std::sin(gamma) -
    ...
    const float cos_zenith = std::clamp(std::sin(latitude) * std::sin(declination) +
    ...
    const float azimuth = std::atan2(std::sin(hour_angle), std::cos(hour_angle) * std::sin(latitude) -
    ...
    const math::vector3f toward_sun{std::sin(world_azimuth) * cos_elevation, std::sin(elevation),
                                    -std::cos(world_azimuth) * cos_elevation};
    return {.azimuth_degrees = std::fmod(math::to_degrees(azimuth) + 360.0f, 360.0f),
                           ...
                           float utc_offset_hours) noexcept
{
    ...
    double cycles =
        (julian_day(year, month, day, local_time_hours - utc_offset_hours) - reference_new_moon) / synodic_month;
...
environment_validation_result validate_world_environment(const world_environment_settings& settings)
{
    ...
    if (!finite(atmosphere.rayleigh_strength) || !finite(atmosphere.mie_strength) ||
        !finite(atmosphere.ozone_strength) || atmosphere.rayleigh_strength < 0.0f || atmosphere.mie_strength < 0.0f ||
        atmosphere.ozone_strength < 0.0f)
        result.errors.emplace_back("atmospheric scattering strengths must be non-negative");
    if (!finite(atmosphere.tint) || !finite(atmosphere.ground_albedo) || !finite(atmosphere.rayleigh_scale_height) ||
        atmosphere.rayleigh_scale_height <= 0.0f || !finite(atmosphere.mie_scale_height) ||
        atmosphere.mie_scale_height <= 0.0f || !finite(atmosphere.multi_scattering_factor) ||
        atmosphere.multi_scattering_factor < 0.0f || !finite(atmosphere.exposure) || atmosphere.exposure < 0.0f ||
    ...
    if (!finite(celestial.latitude_degrees) || celestial.latitude_degrees < -90.0f ||
        celestial.latitude_degrees > 90.0f)
        result.errors.emplace_back("latitude must be in [-90, 90]");
    if (!finite(celestial.longitude_degrees) || celestial.longitude_degrees < -180.0f ||
        celestial.longitude_degrees > 180.0f)
        result.errors.emplace_back("longitude must be in [-180, 180]");
    if (!finite(celestial.utc_offset_hours) || celestial.utc_offset_hours < -14.0f ||
        celestial.utc_offset_hours > 14.0f)
    ...
    if (!finite(celestial.north_offset_degrees) || !finite(celestial.time_scale) || celestial.time_scale < 0.0f ||
        ...
        !finite(celestial.star_intensity) || celestial.star_intensity < 0.0f || !finite(celestial.star_twinkle) ||
        celestial.star_twinkle < 0.0f || celestial.star_twinkle > 1.0f)
    ...
    if (!finite(fog.color) || !finite(fog.density) || fog.density < 0.0f || !finite(fog.height_falloff) ||
        fog.height_falloff < 0.0f || !finite(fog.start_distance) || fog.start_distance < 0.0f ||
...
std::optional<world_environment_settings> read_world_environment_settings(const ecs::world& scene, entity environment)
{
...
{
    if (!scene.alive(environment) || !validate_world_environment(settings).valid) return false;
...
void update_world_environments(ecs::world& scene, float delta_seconds)
{
    scene.view<world_environment_component, celestial_sky_component>().each(
        [&](entity value, const world_environment_component&, const celestial_sky_component&)
        {
            ...
            if (!world_ptr || !celestial_ptr) return;
            auto& world = *world_ptr;
            auto& celestial = *celestial_ptr;
            if (!world.enabled || !celestial.enabled) return;
            if (std::isfinite(delta_seconds) && delta_seconds > 0.0f)
                celestial.animation_time_seconds =
                    std::fmod(celestial.animation_time_seconds + delta_seconds, seconds_per_day);
            if (celestial.time_mode == celestial_time_mode::system_clock)
                use_system_time(celestial);
            else if (celestial.time_mode == celestial_time_mode::simulated && celestial.playing &&
                     std::isfinite(delta_seconds))
            {
                ...
                const auto whole_days =
                    static_cast<std::int32_t>(std::floor(celestial.local_time_hours / hours_per_day));
                celestial.local_time_hours -= static_cast<float>(whole_days) * hours_per_day;
                if (celestial.local_time_hours < 0.0f) celestial.local_time_hours += hours_per_day;
            ...
            if (celestial.sun_mode != sun_position_mode::geographic || !scene.alive(celestial.sun_light)) return;
            const auto solar = calculate_solar_position(
                celestial.latitude_degrees, celestial.longitude_degrees, celestial.north_offset_degrees, celestial.year,
                celestial.month, celestial.day, celestial.local_time_hours, celestial.utc_offset_hours);
            ...
            if (celestial.automatic_sun_light)
            {
                if (auto* light = scene.try_get<directional_light_component>(celestial.sun_light))
                {
                    ...
                    light->temperature_kelvin = std::clamp(
                        (6500.0f - warm * 4000.0f) * celestial.sun_temperature_multiplier, 1000.0f, 40000.0f);
                    light->intensity = clear_day_sun_illuminance_lux * daylight * celestial.sun_intensity_multiplier;
