...
TEST_CASE("M3.4 adjacent evaluation halos share sculpt samples and boundary normals")
{
    ...
    REQUIRE(accumulate_terrain_sculpt_samples(asset, layer,
        std::array{terrain_sculpt_sample_edit{address.region, {address.x, address.z, 7.0f}}}).revision != 0u);
    ...
    const auto left = evaluator.evaluate(asset, {.region = {-1, 0}, .heightfield_source = source,
        .source_bounds = terrain_world_bounds{-8.0, 0.0, -8.0, 12.0, 0.0, 8.0}});
    const auto right = evaluator.evaluate(asset, {.region = {0, 0}, .heightfield_source = source,
