...
/** @brief Load a version-2 project descriptor. @param descriptor_path Descriptor file. @return Parsed descriptor or
 * failure. */
[[nodiscard]] descriptor_result load_descriptor(const std::filesystem::path& descriptor_path);
/** @brief Atomically save a validated descriptor. @param descriptor_path Destination file. @param descriptor Value to
 * save. @return Success or failure. */
[[nodiscard]] project_status save_descriptor(const std::filesystem::path& descriptor_path,
                                             const project_descriptor& descriptor);
/** @brief Validate descriptor structure, compatibility, and optional filesystem state. @param descriptor_path
 * Descriptor identity. @param descriptor Value to validate. @param options Validation policy. @return Writability
 * result or failure. */
...
/** @brief Resolve canonical project-local roots. @param descriptor_path Descriptor file. @param descriptor Parsed
 * descriptor. @return Contained absolute roots or failure. */
[[nodiscard]] context_result resolve_context(const std::filesystem::path& descriptor_path,
                                             const project_descriptor& descriptor);
/** @brief Upgrade a legacy descriptor with a validated backup. @param descriptor_path Legacy descriptor. @param
 * target_engine_version Exact target ARC version. @return Success or failure without partial publication. */
...
/** @brief Discover declarative templates. @param templates_root Installed template root. @return Sorted template
 * snapshots or failure. */
[[nodiscard]] templates_result discover_templates(const std::filesystem::path& templates_root);
/** @brief Generate a complete project atomically through a staging directory. @param request Creation inputs. @return
 * Success or failure. */
...
/** @brief Load and resolve an installation manifest. @param manifest_path Manifest file. @return Resolved installation
 * or failure. */
[[nodiscard]] installation_result load_installation_manifest(const std::filesystem::path& manifest_path);
/** @brief Atomically register an installation manifest. @param registry_path Registry file. @param manifest_path
 * Authority manifest. @return Success or failure. */
[[nodiscard]] project_status register_installation(const std::filesystem::path& registry_path,
                                                   const std::filesystem::path& manifest_path);
/** @brief Remove an installation identity. @param registry_path Registry file. @param installation_id Stable
 * installation ID. @return Success or failure. */
[[nodiscard]] project_status unregister_installation(const std::filesystem::path& registry_path,
                                                     std::string_view installation_id);
/** @brief Resolve every valid registered installation. @param registry_path Registry file. @return Sorted installations
 * or failure. */
[[nodiscard]] installations_result discover_installations(const std::filesystem::path& registry_path);
/** @brief Rebuild a registry from surviving entries and explicit search roots. @param registry_path Registry file.
 * @param search_roots Roots searched for manifests. @return Repaired installations or failure. */
