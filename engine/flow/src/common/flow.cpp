...
bool has_errors(const std::vector<diagnostic>& diagnostics)
{
    return std::any_of(diagnostics.begin(), diagnostics.end(), [](const diagnostic& item) {
        return item.severity == diagnostic_severity::error;
    });
}
...
{
    if (value == "beginPlay")
        return node_kind::begin_play;
    if (value == "endPlay")
        return node_kind::end_play;
    if (value == "tick")
        return node_kind::tick;
    if (value == "fixedTick")
        return node_kind::fixed_tick;
    if (value == "inputAction")
        return node_kind::input_action;
    if (value == "branch")
        return node_kind::branch;
    return std::nullopt;
}
...
std::optional<value_type> parse_value_type(std::string_view value)
{
    ...
    if (value == "entity")
        return value_type::entity;
    if (value == "component")
        return value_type::component;
    return std::nullopt;
}
...
std::optional<flow_value> parse_vector_value(const json& value)
{
...
std::optional<flow_value> parse_default_value(value_type type, const json& value)
{
    ...
    case value_type::boolean:
        if (value.is_boolean())
            return flow_value{value.get<bool>()};
        break;
    case value_type::integer:
        if (value.is_number_integer())
            return flow_value{value.get<std::int64_t>()};
        break;
    case value_type::float32:
        if (value.is_number())
            return flow_value{value.get<double>()};
        break;
    case value_type::vector2:
    ...
    case value_type::name:
        if (value.is_string())
            return flow_value{value.get<std::string>()};
        break;
    case value_type::entity:
    case value_type::component:
        if (value.is_null())
            return flow_value{std::monostate{}};
        break;
    }
    return std::nullopt;
}
...
std::optional<pin_info> output_pin(node_kind kind, std::string_view pin)
{
    ...
    case node_kind::fixed_tick:
        if (pin == "exec")
            return pin_info{.kind = pin_kind::execution};
        if (pin == "deltaSeconds")
            return pin_info{.kind = pin_kind::value, .type = value_type::float32};
        break;
    case node_kind::input_action:
        if (pin == "triggered" || pin == "completed")
            return pin_info{.kind = pin_kind::execution};
        if (pin == "value")
            return pin_info{.kind = pin_kind::value, .type = value_type::float32};
        break;
...
std::optional<pin_info> input_pin(node_kind kind, std::string_view pin)
{
    ...
    if (pin == "exec")
        return pin_info{.kind = pin_kind::execution};
    if (pin == "condition")
        return pin_info{.kind = pin_kind::value, .type = value_type::boolean};
    return std::nullopt;
}
    ...
    for (const json& variable_json : *variables_iterator)
    {
            ...
            !variable_json.contains("exposed") || !variable_json["exposed"].is_boolean())
        {
            add_diagnostic(diagnostics,
                           diagnostic_severity::error,
                           "FLOW_VARIABLE_SCHEMA",
                           "Flow variable requires id, name, defaultValue, and exposed fields.",
                           id);
            continue;
        }
        ...
        if (!parsed_default)
        {
            add_diagnostic(diagnostics,
                           diagnostic_severity::error,
                           "FLOW_VARIABLE_DEFAULT",
                           "Flow variable defaultValue does not match its declared type.",
                           id);
            continue;
        }
    ...
    for (const json& node_json : *nodes_iterator)
    {
        ...
        if (id.empty() || !kind)
        {
            add_diagnostic(diagnostics,
                           diagnostic_severity::error,
                           "FLOW_NODE_TYPE",
                           "Flow node has an empty id or unsupported node type.",
                           id);
            continue;
        }
        ...
        if (values_iterator == node_json.end() || !values_iterator->is_object())
        {
            add_diagnostic(diagnostics,
                           diagnostic_severity::error,
                           "FLOW_NODE_VALUES",
                           "Flow node values must be an object.",
                           id);
            continue;
        }
    ...
    for (const json& connection_json : *connections_iterator)
    {
            ...
            !from_iterator->is_object() || !to_iterator->is_object())
        {
            add_diagnostic(diagnostics,
                           diagnostic_severity::error,
                           "FLOW_CONNECTION_SCHEMA",
                           "Flow connection requires id, kind, from, and to fields.",
                           {},
                           {},
                           id);
            continue;
            ...
            connection.to.pin.empty())
        {
            add_diagnostic(diagnostics,
                           diagnostic_severity::error,
                           "FLOW_CONNECTION_ENDPOINT",
                           "Flow connection endpoints require nodeId and pin.",
                           {},
                           {},
                           id);
            continue;
...
validation_state validate_graph(const source_graph& graph, std::vector<diagnostic>& diagnostics)
{
    ...
    for (const variable& item : graph.variables)
    {
        if (!variable_ids.insert(item.id).second)
            add_diagnostic(diagnostics,
                           diagnostic_severity::error,
                           "FLOW_DUPLICATE_VARIABLE_ID",
                           "Flow variable ids must be unique.",
                           item.id);
    }
    ...
    for (const source_node& node : graph.nodes)
    {
        ...
        if (node.kind == node_kind::input_action)
        {
            ...
            if (action == node.values.end() || !action->is_string() || action->get<std::string>().empty())
                add_diagnostic(diagnostics,
                               diagnostic_severity::error,
                               "FLOW_INPUT_ACTION_NAME",
                               "Input Action nodes require a non-empty action name.",
                               node.id);
        }
    }
    ...
    for (const source_connection& connection : graph.connections)
    {
        if (!connection_ids.insert(connection.id).second)
            add_diagnostic(diagnostics,
                           diagnostic_severity::error,
                           "FLOW_DUPLICATE_CONNECTION_ID",
                           "Flow connection ids must be unique.",
                           {},
                           {},
                           connection.id);
        ...
        if (from_node_iterator == state.nodes.end() || to_node_iterator == state.nodes.end())
        {
            add_diagnostic(diagnostics,
                           diagnostic_severity::error,
                           "FLOW_CONNECTION_NODE",
                           "Flow connection references a node that does not exist.",
                           {},
                           {},
                           connection.id);
            continue;
        ...
        if (connection.from.node_id == connection.to.node_id)
        {
            add_diagnostic(diagnostics,
                           diagnostic_severity::error,
                           "FLOW_SELF_CONNECTION",
                           "Flow nodes cannot connect to themselves.",
        ...
        if (!from_pin || !to_pin)
        {
            add_diagnostic(diagnostics,
                           ...
                           !from_pin ? connection.from.pin : connection.to.pin,
                           connection.id);
            continue;
        }
        ...
        if (from_pin->kind != expected_kind || to_pin->kind != expected_kind)
        {
            add_diagnostic(diagnostics,
                           diagnostic_severity::error,
                           "FLOW_CONNECTION_KIND",
                           "Flow connection kind does not match its endpoint pins.",
        ...
        if (expected_kind == pin_kind::value && from_pin->type != to_pin->type)
        {
            add_diagnostic(diagnostics,
                           diagnostic_severity::error,
                           "FLOW_VALUE_TYPE_MISMATCH",
                           "Flow value connection types are incompatible.",
                           connection.to.node_id,
                           connection.to.pin,
                           connection.id);
            continue;
        ...
        if (!state.incoming.emplace(incoming_key, &connection).second)
            add_diagnostic(diagnostics,
                           diagnostic_severity::error,
                           "FLOW_MULTIPLE_INPUTS",
                           "A Flow input pin can have only one incoming connection.",
                           connection.to.node_id,
                           connection.to.pin,
                           connection.id);
    ...
    for (const source_node& node : graph.nodes)
    {
        if (visits.find(node.id) == visits.end() && !visit(node.id))
            break;
    }
    ...
    for (const source_node& node : graph.nodes)
    {
        if (node.kind == node_kind::branch && reachable.find(node.id) == reachable.end())
            add_diagnostic(diagnostics,
                           ...
                           "Flow node is not reachable from an event entry point.",
                           node.id);
    }
...
std::vector<const source_node*> sorted_nodes(const source_graph& graph)
{
    ...
    std::sort(result.begin(), result.end(), [](const source_node* left, const source_node* right) {
...
ir_program build_ir(const source_graph& graph, const validation_state& validation)
{
    ...
    std::sort(program.variables.begin(), program.variables.end(), [](const variable& left, const variable& right) {
        return left.id < right.id;
    });
    ...
    const auto execution_target = [&](const source_node& node, std::string_view pin) {
        ...
        if (connection == validation.execution_outgoing.end())
    ...
    const auto add_binding = [&](ir_entry_point& entry,
                                 const source_node& node,
                                 std::string_view pin,
                                 entry_value_kind source) {
