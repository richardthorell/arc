...
public:
    ...
    constexpr explicit point(T value) noexcept : values_(value) {}

    /**
     * @brief Construct from exactly `N` coordinate values.
     * @throws std::length_error when the initializer length does not match `N`.
     */
    constexpr point(std::initializer_list<T> values) : values_(values) {}

    /// @brief Construct from a backing vector.
    constexpr explicit point(const vector_type& values) noexcept : values_(values) {}
    ...
    constexpr explicit point(const Expr& expr) noexcept : values_(expr)
    {
    }
