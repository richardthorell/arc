...
{
    arc::memory::tracked_memory_resource resource("test");
    {
...
{
    auto& resource = arc::memory::default_tracked_memory_resource();
    ...
    const auto stats = arc::memory::default_memory_stats();
    REQUIRE(stats.allocation_count == 1);
    REQUIRE(stats.deallocation_count == 1);
    REQUIRE(stats.bytes_outstanding == 0);
}
...
TEST_CASE("memory system tracks domains tags and world leaks")
{
    ...
    REQUIRE(active.tags.front().domain == arc::memory::memory_domain::world);
...
TEST_CASE("soft memory pressure is reported once until usage recovers")
{
    ...
    system.deallocate(second, 16, 16, arc::memory::memory_domain::streaming);
    system.deallocate(first, 80, 16, arc::memory::memory_domain::streaming);

    void* third = system.allocate(80, 16, arc::memory::memory_domain::streaming);
    REQUIRE(soft_events == 2);
    system.deallocate(third, 80, 16, arc::memory::memory_domain::streaming);
}

TEST_CASE("linear arena growth preserves pointers and supports marks")
{
    arc::memory::linear_arena arena(64);
...
{
    const auto original = arc::memory::current_memory_tag();
    {
        arc::memory::allocation_tag_scope outer("outer");
        REQUIRE(arc::memory::current_memory_tag().name == "outer");
        {
            arc::memory::allocation_tag_scope inner("inner");
            REQUIRE(arc::memory::current_memory_tag().name == "inner");
        }
        REQUIRE(arc::memory::current_memory_tag().name == "outer");
    }
    REQUIRE(arc::memory::current_memory_tag().id == original.id);
}
...
TEST_CASE("world memory contexts tag allocations with stable world identity")
{
    ...
    REQUIRE(leaks.front().domain == arc::memory::memory_domain::components);
}
