...
TEST_CASE("integer shifts", "[simd][game]")
{
    ...
    int32_t out[4]{};
    arc::simd::store_unaligned(out, arc::simd::shift_left<1>(v));
    REQUIRE(out[0] == 2);
    REQUIRE(out[1] == -16);
    REQUIRE(out[2] == 32);
    REQUIRE(out[3] == -64);

    arc::simd::store_unaligned(out, arc::simd::shift_right_arithmetic<1>(v));
    REQUIRE(out[0] == 0);
    REQUIRE(out[1] == -4);
    REQUIRE(out[2] == 8);
    REQUIRE(out[3] == -16);
...
TEST_CASE("conversions and bit cast", "[simd][game]")
{
    ...
    int32_t int_out[4]{};
    arc::simd::store_unaligned(int_out, i);
    REQUIRE(int_out[0] == 1);
    REQUIRE(int_out[3] == 4);

    auto bits = arc::simd::bit_cast<uint32_t>(f);
    uint32_t bit_out[4]{};
    arc::simd::store_unaligned(bit_out, bits);
...
TEST_CASE("mask bits and compression", "[simd][game]")
{
    ...
    REQUIRE(arc::simd::mask_to_bits(mask) == 0b0101);
    REQUIRE(arc::simd::popcount(mask) == 2);
    REQUIRE(arc::simd::first_active_lane(mask) == 0);

    float out[4]{};
    arc::simd::store_unaligned(out, arc::simd::compress(v, mask, -1.0f));
    REQUIRE(out[0] == 10.0f);
    REQUIRE(out[1] == 30.0f);
    REQUIRE(out[2] == -1.0f);
    REQUIRE(out[3] == -1.0f);
...
TEST_CASE("gather and scatter", "[simd][game]")
{
    ...
    float out[4]{};
    arc::simd::store_unaligned(out, gathered);
    REQUIRE(out[0] == 30.0f);
    REQUIRE(out[1] == 10.0f);
    REQUIRE(out[2] == 70.0f);
    REQUIRE(out[3] == 0.0f);
    ...
    arc::simd::scatter(destination.data(), indices, gathered);
...
TEST_CASE("geometry helpers", "[simd][game]")
{
    ...
    REQUIRE(arc::simd::dot3(x, y) == 0.0f);

    float out[4]{};
    arc::simd::store_unaligned(out, arc::simd::cross3(x, y));
    REQUIRE(out[0] == 0.0f);
    REQUIRE(out[1] == 0.0f);
    REQUIRE(out[2] == 1.0f);
    REQUIRE(out[3] == 0.0f);
...
TEST_CASE("transpose and transform", "[simd][game]")
{
    ...
    float out[4]{};
    arc::simd::store_unaligned(out, columns[0]);
    REQUIRE(out[0] == 1.0f);
    REQUIRE(out[1] == 5.0f);
    REQUIRE(out[2] == 9.0f);
    REQUIRE(out[3] == 13.0f);
...
TEST_CASE("transcendentals and packing", "[simd][game]")
{
    ...
    float out[4]{};
    arc::simd::store_unaligned(out, arc::simd::sin(v));
    REQUIRE(out[0] == Catch::Approx(0.0f));

    auto packed = arc::simd::pack_unorm8(v);
    uint32_t packed_out[4]{};
    arc::simd::store_unaligned(packed_out, packed);
    REQUIRE(packed_out[0] == 0u);
    REQUIRE(packed_out[1] == 128u);
    REQUIRE(packed_out[2] == 255u);
    REQUIRE(packed_out[3] == 255u);

    auto half = arc::simd::float_to_half(v);
    auto unpacked_half = arc::simd::half_to_float(half);
    arc::simd::store_unaligned(out, unpacked_half);
    REQUIRE(out[0] == Catch::Approx(0.0f));
...
TEST_CASE("integer saturation and quaternion rotation", "[simd][game]")
{
    ...
    uint32_t uint_out[4]{};
    arc::simd::store_unaligned(uint_out, arc::simd::saturating_add(a, b));
    REQUIRE(uint_out[0] == 0xFFFFFFFFu);
    REQUIRE(uint_out[1] == 30u);

    arc::simd::store_unaligned(uint_out, arc::simd::saturating_sub(a, b));
    ...
    float out[4]{};
    arc::simd::store_unaligned(out, arc::simd::quat_rotate3(q, v));
