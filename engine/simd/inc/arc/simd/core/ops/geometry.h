...
template <class T, std::size_t N> constexpr T dot3(const simd<T, N>& a, const simd<T, N>& b) noexcept
{
    static_assert(N >= 3, "dot3 requires at least 3 lanes");
    return extract<0>(a) * extract<0>(b) + extract<1>(a) * extract<1>(b) + extract<2>(a) * extract<2>(b);
}

/// @brief Return the dot product of the first four lanes.
template <class T, std::size_t N> constexpr T dot4(const simd<T, N>& a, const simd<T, N>& b) noexcept
{
...
template <class T, std::size_t N> constexpr T length_squared3(const simd<T, N>& value) noexcept
{
    return dot3(value, value);
}

/// @brief Return squared length using the first four lanes.
template <class T, std::size_t N> constexpr T length_squared4(const simd<T, N>& value) noexcept
{
    return dot4(value, value);
}

/// @brief Return length using the first three lanes.
template <class T, std::size_t N> inline T length3(const simd<T, N>& value) noexcept
{
    return static_cast<T>(std::sqrt(length_squared3(value)));
}

/// @brief Return length using the first four lanes.
template <class T, std::size_t N> inline T length4(const simd<T, N>& value) noexcept
{
    return static_cast<T>(std::sqrt(length_squared4(value)));
}

/// @brief Normalize the first three lanes, using `fallback` for zero length.
template <class T, std::size_t N> inline simd<T, N> normalize3(const simd<T, N>& value, T fallback = T{}) noexcept
{
...
template <class T, std::size_t N> inline simd<T, N> normalize4(const simd<T, N>& value, T fallback = T{}) noexcept
{
    auto lanes = detail::simd_to_array(value);
...
template <class T, std::size_t N> constexpr simd<T, N> cross3(const simd<T, N>& a, const simd<T, N>& b) noexcept
{
    ...
    if constexpr (N >= 4) result[3] = T{};
    return detail::simd_from_array<T, N>(result);
}

/// @brief Linearly interpolate between two SIMD vectors.
template <class T, std::size_t N> constexpr simd<T, N> lerp(const simd<T, N>& a, const simd<T, N>& b, T t) noexcept
{
...
constexpr simd<T, N> project3(const simd<T, N>& value, const simd<T, N>& onto) noexcept
{
    ...
    if (denom == T{}) return fill<T, N>(T{});
    return onto * fill<T, N>(dot3(value, onto) / denom);
}

/// @brief Transpose four SIMD rows that represent a 4x4 matrix.
template <class T> constexpr std::array<simd<T, 4>, 4> transpose4x4(const std::array<simd<T, 4>, 4>& rows) noexcept
{
    std::array<simd<T, 4>, 4> result{};
    result[0] = detail::simd_from_array<T, 4>(
        {extract<0>(rows[0]), extract<0>(rows[1]), extract<0>(rows[2]), extract<0>(rows[3])});
    result[1] = detail::simd_from_array<T, 4>(
        {extract<1>(rows[0]), extract<1>(rows[1]), extract<1>(rows[2]), extract<1>(rows[3])});
    result[2] = detail::simd_from_array<T, 4>(
        {extract<2>(rows[0]), extract<2>(rows[1]), extract<2>(rows[2]), extract<2>(rows[3])});
    result[3] = detail::simd_from_array<T, 4>(
        {extract<3>(rows[0]), extract<3>(rows[1]), extract<3>(rows[2]), extract<3>(rows[3])});
    return result;
}
...
constexpr simd<T, 4> transform4(const std::array<simd<T, 4>, 4>& rows, const simd<T, 4>& value) noexcept
{
    return detail::simd_from_array<T, 4>(
        {dot4(rows[0], value), dot4(rows[1], value), dot4(rows[2], value), dot4(rows[3], value)});
}
