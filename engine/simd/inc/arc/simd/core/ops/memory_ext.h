...
template <class T, std::size_t N> constexpr void stream_store(T* ptr, const simd<T, N>& value) noexcept
{
    store_aligned(ptr, value);
}

template <class T, std::size_t N> constexpr simd<T, N> gather(const T* base, const simd<int32_t, N>& indices) noexcept
{
...
constexpr simd<T, N> gather(const T* base, const simd<int32_t, N>& indices, const simd_mask<N>& mask,
...
template <class T, std::size_t N>
constexpr void scatter(T* base, const simd<int32_t, N>& indices, const simd<T, N>& value,
                       const simd_mask<N>& mask) noexcept
{
    auto index_values = detail::simd_to_array(indices);
    auto values = detail::simd_to_array(value);
    const auto bits = mask_to_bits(mask);
    for (std::size_t i = 0; i < N; ++i)
        if ((bits & (std::uint64_t{1} << i)) != 0) base[index_values[i]] = values[i];
}

template <class T> constexpr simd<T, 4> load3(const T* ptr, T w = T{}) noexcept
{
    return detail::simd_from_array<T, 4>({ptr[0], ptr[1], ptr[2], w});
}

template <class T> constexpr void store3(T* ptr, const simd<T, 4>& value) noexcept
{
...
template <class T> constexpr simd<T, 4> load4(const T* ptr) noexcept
{
    return detail::simd_from_array<T, 4>({ptr[0], ptr[1], ptr[2], ptr[3]});
}

template <class T> constexpr void store4(T* ptr, const simd<T, 4>& value) noexcept
{
    auto lanes = detail::simd_to_array(value);
    ptr[0] = lanes[0];
    ptr[1] = lanes[1];
