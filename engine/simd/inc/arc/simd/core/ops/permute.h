...
template <std::size_t... I, class T, std::size_t N> constexpr simd<T, N> permute(const simd<T, N>& value) noexcept
{
...
template <class T, std::size_t N> constexpr simd<T, N> reverse(const simd<T, N>& value) noexcept
{
    return [&]<std::size_t... I>(std::index_sequence<I...>)
    { return permute<N - 1 - I...>(value); }(std::make_index_sequence<N>{});
}

template <std::size_t Offset, class T, std::size_t N> constexpr simd<T, N> rotate_left(const simd<T, N>& value) noexcept
{
    static_assert(Offset < N, "rotate_left offset must be less than N");

    return [&]<std::size_t... I>(std::index_sequence<I...>)
    { return permute<(I + Offset) % N...>(value); }(std::make_index_sequence<N>{});
}
...
constexpr simd<T, N> rotate_right(const simd<T, N>& value) noexcept
{
    ...
    return [&]<std::size_t... I>(std::index_sequence<I...>)
    { return permute<(I + N - Offset) % N...>(value); }(std::make_index_sequence<N>{});
}
