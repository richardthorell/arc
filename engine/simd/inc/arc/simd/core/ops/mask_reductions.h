 ...
 */
template <std::size_t N> constexpr bool any(const simd_mask<N>& mask) noexcept
{
 ...
 */
template <std::size_t N> constexpr bool all(const simd_mask<N>& mask) noexcept
{
    using detail::simd_access;
 ...
 */
template <std::size_t N> constexpr bool none(const simd_mask<N>& mask) noexcept
{
    return !any(mask);
}
