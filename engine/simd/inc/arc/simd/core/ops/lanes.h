 ...
 */
template <std::size_t I, class T, std::size_t N> constexpr T extract(const simd<T, N>& value) noexcept
{
...
constexpr simd<T, N> insert(const simd<T, N>& value, T lane_value) noexcept
{
    ...
    return [&]<std::size_t... Index>(std::index_sequence<Index...>)
    {
        return simd_access::template make_simd<T, N>(
            (Index == block_index
                 ? ops_for<simd<T, N>>::template insert<lane_index>(simd_access::block(value, Index), lane_value)
                 : simd_access::block(value, Index))...);
 ...
 */
template <std::size_t I, class T, std::size_t N> constexpr simd<T, N> broadcast(const simd<T, N>& value) noexcept
{
