 ...
 */
template <class T, std::size_t N> constexpr simd<T, N> min(const simd<T, N>& a, const simd<T, N>& b) noexcept
{
 ...
 */
template <class T, std::size_t N> constexpr simd<T, N> max(const simd<T, N>& a, const simd<T, N>& b) noexcept
{
    if constexpr (std::is_unsigned_v<T>)
        return detail::simd_map(a, b, [](T x, T y) { return x > y ? x : y; });
    else
