 ...
 */
template <typename SimdOrMask> using ops_for = typename SimdOrMask::op_type;
...
private:
    ...
    template <std::size_t M> friend struct simd_mask;

    friend struct detail::simd_access;

    template <std::size_t... Index, class Op>
    constexpr explicit simd(std::index_sequence<Index...>, Op op) noexcept : data{op(Index)...}
    {
    }

    template <class... Blocks> constexpr explicit simd(Blocks... blocks) noexcept : data{blocks...} {}
