 ...
 */
template <class T> struct simd_op;

/**
 * @brief Maximum number of lanes for a given data type based on available SIMD extensions.
 */
template <class T> inline constexpr std::size_t simd_max_lanes = 1;
 ...
 * @tparam N The number of lanes in the SIMD register.
 */
template <class T, std::size_t N> using simd_register_t = typename simd_register<T, N>::type;
