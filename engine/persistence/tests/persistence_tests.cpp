...
public:
    temporary_directory()
    {
        path = std::filesystem::temp_directory_path() /
               ("arc-persistence-" + std::to_string(std::chrono::steady_clock::now().time_since_epoch().count()));
...
TEST_CASE("reflected JSON preserves unknown fields and verifies integrity")
{
    ...
    const auto unknown =
        std::find_if(fields.begin(), fields.end(), [](const auto& field) { return field.name == "futureField"; });
...
{
    const auto document = make_document();
    const auto first_result = arc::persistence::write_tagged_binary(document, "windows-x64-vulkan12");
    const auto second_result = arc::persistence::write_tagged_binary(document, "windows-x64-vulkan12");
...
TEST_CASE("migration registry applies consecutive component and document migrations")
{
    ...
    REQUIRE(migrations.register_document(arc::persistence::document_kind::scene, 1, 2,
                                         [](auto& document)
                                         {
                                             document.name += " v2";
                                             return arc::persistence::persistence_status::success();
                                         }));
    REQUIRE(migrations.register_document(arc::persistence::document_kind::scene, 2, 3,
                                         [](auto& document)
                                         {
                                             document.name += " v3";
                                             return arc::persistence::persistence_status::success();
