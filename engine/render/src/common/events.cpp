...
render_event_type render_event::type() const noexcept
{
    ...
    if (std::holds_alternative<viewport_resize_event>(payload)) return render_event_type::viewport_resize;
    if (std::holds_alternative<draw_mesh_event>(payload)) return render_event_type::draw;
    if (std::holds_alternative<directional_light_event>(payload)) return render_event_type::directional_light;
...
render_event_writer::render_event_writer(render_event_buffer& buffer) noexcept : buffer_(&buffer) {}
...
void render_event_writer::environment_upload(environment_handle handle,
                                             std::shared_ptr<const environment_descriptor> environment,
                                             std::string label)
{
    render_event event{};
    event.payload =
        environment_upload_event{.handle = handle, .environment = std::move(environment), .label = std::move(label)};
    buffer_->push(std::move(event));
}
...
void render_event_writer::draw_mesh(mesh_handle mesh, material_handle material, const math::matrix4f& model,
                                    const math::matrix4f& view_projection, std::string label)
{
    draw_mesh_tinted(mesh, material, model, view_projection, render_mode::shaded, mesh_visualization_mode::standard,
                     false, math::vector4f::one, math::vector4f{0.25f, 0.65f, 1.0f, 1.0f}, std::move(label));
}

void render_event_writer::draw_mesh(mesh_handle mesh, material_handle material, const math::matrix4f& model,
                                    const math::matrix4f& view_projection, render_mode mode,
                                    mesh_visualization_mode visualization, bool selected,
                                    const math::vector4f& wire_color, std::string label)
{
    draw_mesh_tinted(mesh, material, model, view_projection, mode, visualization, selected, math::vector4f::one,
                     wire_color, std::move(label));
}

void render_event_writer::draw_mesh_tinted(mesh_handle mesh, material_handle material, const math::matrix4f& model,
                                           const math::matrix4f& view_projection, render_mode mode,
                                           mesh_visualization_mode visualization, bool selected,
                                           const math::vector4f& base_color_tint, const math::vector4f& wire_color,
                                           std::string label)
{
    render_event event{};
    event.payload = draw_mesh_event{.mesh = mesh,
                                    .material = material,
                                    .model = model,
...
void render_event_writer::directional_light(const math::vector3f& direction, const math::vector3f& color,
                                            float intensity, bool casts_shadows, std::string label, bool enabled,
                                            bool use_color_temperature, float temperature_kelvin,
                                            light_intensity_unit intensity_unit, texture_handle cookie_texture,
                                            shadow_settings shadow)
{
    render_event event{};
    event.payload = directional_light_event{.direction = direction,
                                            .color = color,
                                            .intensity = intensity,
...
void render_event_writer::point_light(const math::vector3f& position, const math::vector3f& color, float intensity,
                                      float range, bool casts_shadows, std::string label, bool enabled,
                                      bool use_color_temperature, float temperature_kelvin,
                                      light_intensity_unit intensity_unit, texture_handle cookie_texture,
                                      shadow_settings shadow)
{
    render_event event{};
    event.payload = point_light_event{.position = position,
                                      .color = color,
                                      .intensity = intensity,
                                      .range = range,
                                      .casts_shadows = casts_shadows,
...
void render_event_writer::spot_light(const math::vector3f& position, const math::vector3f& direction,
                                     const math::vector3f& color, float intensity, float range, float inner_angle,
                                     float outer_angle, bool casts_shadows, std::string label, bool enabled,
                                     bool use_color_temperature, float temperature_kelvin,
                                     light_intensity_unit intensity_unit, texture_handle cookie_texture,
                                     shadow_settings shadow)
{
    render_event event{};
    event.payload = spot_light_event{.position = position,
                                     .direction = direction,
                                     .color = color,
                                     .intensity = intensity,
                                     .range = range,
...
void render_event_writer::area_light(const math::vector3f& position, const math::vector3f& direction,
                                     const math::vector3f& tangent, const math::vector3f& color, float intensity,
                                     float width, float height, area_light_shape shape, bool two_sided,
                                     bool casts_shadows, std::string label, bool enabled, bool use_color_temperature,
                                     float temperature_kelvin, light_intensity_unit intensity_unit,
                                     shadow_settings shadow)
{
    render_event event{};
    event.payload = area_light_event{.position = position,
                                     .direction = direction,
                                     .tangent = tangent,
                                     .color = color,
                                     .intensity = intensity,
...
void render_event_writer::gpu_scene_update(std::shared_ptr<const gpu_scene_update_batch> batch)
{
    render_event event{};
