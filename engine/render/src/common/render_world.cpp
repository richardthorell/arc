                                   ...
                                   const world_environment_data& environment)
{
    ...
    const auto water_frequency_fields = graph.add_resource({.name = "water_frequency_fields",
                                                            ...
                                                            .byte_size = 16u,
                                                            .element_stride = 16u});
    const auto water_spatial_fields = graph.add_resource({.name = "water_spatial_fields",
                                                          .kind = render_resource_kind::buffer,
                                                          .byte_size = 16u,
                                                          .element_stride = 16u});
    const auto water_surface_fields = graph.add_resource({.name = "water_surface_fields",
                                                          ...
                                                          .byte_size = 16u,
                                                          .element_stride = 16u});
    const auto water_foam_history = graph.add_resource({.name = "water_foam_history",
                                                        .kind = render_resource_kind::buffer,
                                                        .byte_size = 16u,
                                                        .element_stride = 16u,
                                                        .persistent_key = "water.foam_history",
                                                        .history_length = 2,
                                                        .history_reset = render_history_reset::world_epoch_change});
    ...
    graph.add_pass({.name = "Water inverse FFT",
                    ...
                    .reads = {{.handle = water_frequency_fields,
                               ...
                               .usage = render_resource_usage::storage_buffer,
                               .stages = render_pipeline_stage::compute_shader}},
                    .writes = {{.handle = water_spatial_fields,
                                .kind = render_resource_kind::buffer,
                                .usage = render_resource_usage::storage_buffer,
                                .stages = render_pipeline_stage::compute_shader,
                                .write = true}}});
    graph.add_pass({.name = "Water foam update",
                    .queue = compute_queue,
                    .kind = render_pass_kind::compute,
                    .builtin = builtin_render_pass::water_foam_update,
                    .reads = {{.handle = water_spatial_fields,
                               .kind = render_resource_kind::buffer,
                               .usage = render_resource_usage::storage_buffer,
                               .stages = render_pipeline_stage::compute_shader},
                              {.handle = water_foam_history,
                               .kind = render_resource_kind::buffer,
                               .usage = render_resource_usage::storage_buffer,
                               .stages = render_pipeline_stage::compute_shader,
