...
bool resources_compatible(const render_graph_resource& lhs, const render_graph_resource& rhs) noexcept
{
    ...
    return lhs.kind == rhs.kind && lhs.dimension == rhs.dimension && lhs.format == rhs.format &&
           lhs.memory == rhs.memory && lhs.extent.width == rhs.extent.width && lhs.extent.height == rhs.extent.height &&
           lhs.extent.depth == rhs.extent.depth && lhs.extent_mode == rhs.extent_mode &&
           lhs.width_scale == rhs.width_scale && lhs.height_scale == rhs.height_scale &&
           lhs.mip_levels == rhs.mip_levels && lhs.array_layers == rhs.array_layers &&
...
render_graph_compile_result render_graph::compile(const render_graph_compile_options& options) const
{
    ...
    try
    {
        ...
        for (auto& resource : resolved_resources)
        {
            ...
            {
                const auto base = resource.extent_mode == render_extent_mode::relative_to_output ? options.output_extent
                                  : options.render_extent.width != 0 && options.render_extent.height != 0
                                      ? options.render_extent
                                      : options.output_extent;
                if (base.width != 0 && base.height != 0)
                {
                    resource.extent.width = std::max(1u, static_cast<std::uint32_t>(std::ceil(
                                                             static_cast<float>(base.width) * resource.width_scale)));
                    resource.extent.height = std::max(
                        1u,
                        static_cast<std::uint32_t>(std::ceil(static_cast<float>(base.height) * resource.height_scale)));
                                           ...
                                           std::uint32_t pass_index, render_queue_type queue)
        {
            ...
            const auto stages = access.stages != render_pipeline_stage::none
                                    ? access.stages
                                    : (queue == render_queue_type::compute    ? render_pipeline_stage::compute_shader
        ...
        for (const auto& transition : result.transitions)
        {
            ...
            const bool duplicate =
                std::any_of(waits.begin(), waits.end(), [&](const render_queue_wait& existing)
        ...
        for (std::uint32_t index = 0; index < resources_.size(); ++index)
        {
            ...
            result.history_rotations.push_back(
                {.handle = {index},
                 .persistent_key = resource.persistent_key,
                 .history_length = resource.history_length,
                 .reset = resource.history_reset,
                 .invalidated = (resource.history_reset & options.temporal_reset) != render_history_reset::none});
        }
