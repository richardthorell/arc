                                                           ...
                                                           const shader_compile_request& request)
{
    ...
    {
        if (!current_source || found->second.source == current_source) return found->second.result;
    }

    auto result = compiler.compile(request);
    if (result) cache_.insert_or_assign(key, cached_shader{.result = result, .source = current_source});
    return result;
}
