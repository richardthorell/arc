...
shadow_atlas_allocator::shadow_atlas_allocator(std::uint32_t atlas_size, std::uint32_t minimum_tile_size,
                                               std::uint32_t guard_texels)
    : atlas_size_(floor_power_of_two(std::max(1u, atlas_size))),
      minimum_tile_size_(floor_power_of_two(std::max(1u, minimum_tile_size))), guard_texels_(guard_texels)
{
...
{
    if (x + cells > cells_per_axis_ || y + cells > cells_per_axis_) return false;
    for (std::uint32_t row = y; row < y + cells; ++row)
        for (std::uint32_t column = x; column < x + cells; ++column)
            if (occupancy_[static_cast<std::size_t>(row) * cells_per_axis_ + column] != 0) return false;
    return true;
}
...
std::optional<shadow_atlas_allocation> shadow_atlas_allocator::try_allocate(const shadow_atlas_request& request,
                                                                            std::uint32_t resolution)
{
    ...
    const std::uint32_t tile_size = std::max(minimum_tile_size_, std::bit_ceil(resolution));
    if (tile_size > atlas_size_) return std::nullopt;

    const std::uint32_t cells = tile_size / minimum_tile_size_;
    const std::uint32_t face_count = request.kind == shadow_light_kind::point ? point_shadow_face_count : 1u;
    ...
    for (std::uint32_t face = 0; face < face_count; ++face)
    {
        ...
        for (std::uint32_t y = 0; y + cells <= cells_per_axis_ && !found; y += cells)
        {
            ...
            {
                if (!region_free(x, y, cells)) continue;
                faces[face] = {.x = x * minimum_tile_size_,
    ...
    slot.occupied = true;
    slot.allocation = {.handle = {index, slot.generation},
                       .kind = request.kind,
...
std::optional<std::size_t> shadow_atlas_allocator::eviction_candidate(std::uint16_t incoming_priority,
                                                                      std::uint64_t protected_light_key) const noexcept
{
    ...
    for (std::size_t index = 0; index < slots_.size(); ++index)
    {
        ...
        if (!slot.occupied || slot.allocation.light_key == protected_light_key ||
            ...
            continue;
        if (!candidate || slot.allocation.priority < slots_[*candidate].allocation.priority ||
...
std::optional<shadow_atlas_allocation> shadow_atlas_allocator::allocate(const shadow_atlas_request& request)
{
    ...
    const std::uint32_t maximum =
        std::min(atlas_size_, std::bit_ceil(std::max(request.minimum_resolution, request.requested_resolution)));
    const std::uint32_t minimum =
        std::max(minimum_tile_size_ > guard_texels_ * 2u ? minimum_tile_size_ - guard_texels_ * 2u : 1u,
    ...
    for (std::uint32_t resolution = maximum; resolution >= minimum; resolution /= 2u)
    {
        ...
        {
            if (resolution < maximum) ++resolution_reduction_count_;
            return allocation;
        }
...
const shadow_atlas_allocation* shadow_atlas_allocator::find_light(shadow_light_kind kind,
...
bool shadow_atlas_allocator::touch(shadow_allocation_handle handle, std::uint64_t frame_index,
...
{
    shadow_atlas_statistics result{.atlas_size = atlas_size_,
    ...
    for (const auto value : occupancy_)
        if (value != 0) result.occupied_texels += minimum_tile_size_ * minimum_tile_size_;
    for (const auto& slot : slots_)
        result.allocation_count += slot.occupied ? 1u : 0u;
    return result;
}

directional_shadow_layout fit_directional_shadow_cascades(const directional_shadow_camera& camera,
                                                          ...
                                                          std::uint32_t resolution) noexcept
{
    ...
    result.cascade_count = std::clamp(settings.cascade_count, 1u, maximum_directional_shadow_cascades);
    resolution = std::max(1u, resolution);

    const float near_plane = std::max(0.001f, camera.near_plane);
    const float far_plane =
        std::max(near_plane + 0.001f, std::min(camera.far_plane, std::max(near_plane, settings.maximum_distance)));
    ...
    const math::vector3f up = std::abs(math::dot(light_direction, math::vector3f{0.0f, 1.0f, 0.0f})) > 0.95f
    ...
    for (std::uint32_t cascade_index = 0; cascade_index < result.cascade_count; ++cascade_index)
    {
        ...
        if (settings.stable) radius = std::ceil(radius * 16.0f) / 16.0f;
        ...
        if (settings.stable && texel_size > 0.0f)
        {
            ...
            center = math::add(center, math::vector3f{light_basis(0, 0) * (snapped_x - light_center[0]) +
        ...
        const auto view = look_at_rh(math::sub(center, math::mul(light_direction, radius * 2.5f)), center, up);
        ...
        result.cascades[cascade_index] = {.light_view_projection = math::matmul(projection, view),
                                          .center = center,
                                          .radius = radius,
