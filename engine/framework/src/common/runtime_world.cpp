...
constexpr int role_order(runtime_world_role role) noexcept
{
    switch (role)
    {
        case runtime_world_role::server:
            return 0;
        case runtime_world_role::client:
            return 1;
        case runtime_world_role::editor_preview:
            return 2;
    }
    return 3;
}
...
const char* placeholder_name(ecs::system_phase phase) noexcept
{
    switch (phase)
    {
        case ecs::system_phase::input:
            return "ARC Input Boundary";
        case ecs::system_phase::network_receive:
            return "ARC Network Receive TODO";
        case ecs::system_phase::gameplay_commands:
            return "ARC Gameplay Commands TODO";
        case ecs::system_phase::movement:
            return "ARC Movement TODO";
        case ecs::system_phase::physics:
            return "ARC Physics TODO";
        case ecs::system_phase::abilities:
            return "ARC Abilities TODO";
        case ecs::system_phase::ai:
            return "ARC AI TODO";
        case ecs::system_phase::replication:
            return "ARC Replication TODO";
        case ecs::system_phase::presentation_extraction:
            return "ARC Presentation Extraction Boundary";
    }
...
{
public:
    explicit execution_scope(bool& executing) noexcept : executing_(&executing)
    {
        executing = true;
    }
...
runtime_world::runtime_world(memory::memory_system& memory, runtime_world_id id, runtime_world_descriptor descriptor)
    : id_(id), descriptor_(std::move(descriptor)), owned_entities_(memory, id.value, descriptor_.memory),
      entities_(&owned_entities_)
{
    if (descriptor_.name.empty()) descriptor_.name = "World " + std::to_string(id.value);
    if (descriptor_.seed == 0) descriptor_.seed = ecs::splitmix64(id.value);
    if (descriptor_.install_placeholder_systems) install_placeholder_systems();
}

void runtime_world::install_placeholder_systems()
{
    constexpr std::array phases{
        ecs::system_phase::input,    ecs::system_phase::network_receive, ecs::system_phase::gameplay_commands,
        ecs::system_phase::movement, ecs::system_phase::physics,         ecs::system_phase::abilities,
        ecs::system_phase::ai,       ecs::system_phase::replication,     ecs::system_phase::presentation_extraction};
    for (const ecs::system_phase phase : phases)
    {
        systems_.add({.name = placeholder_name(phase),
                      .phase = phase,
...
{
    if (state_ == runtime_world_state::running || state_ == runtime_world_state::paused) return;
    ...
    if (!schedule_errors.empty())
    {
        ...
        throw std::invalid_argument("runtime world '" + descriptor_.name +
...
{
    if (state_ == runtime_world_state::stopped) return;
...
{
    if (state_ == runtime_world_state::paused) state_ = runtime_world_state::running;
}
...
runtime_world_run_result runtime_world::run_phase(jobs::job_system& jobs, ecs::system_phase phase,
                                                  const ecs::system_execution_info& execution)
{
    ...
    if (!result.succeeded()) fault(result.errors.front().message);
    return result;
}

runtime_world_run_result runtime_world::run_fixed(jobs::job_system& jobs, const simulation_tick& tick,
                                                  runtime_service_provider* services,
                                                  const simulation_input_snapshot* input,
                                                  memory::tick_arena& tick_memory, memory::frame_arena& frame_memory,
                                                  std::uint64_t process_seed)
{
    runtime_world_run_result aggregate;
    if (state_ != runtime_world_state::running) return aggregate;
    ...
    for (const ecs::system_phase phase : ecs::fixed_system_phases)
    {
        ...
        if (!phase_result.succeeded()) return aggregate;
    }
    last_completed_tick_ = tick.id;
    return aggregate;
}

runtime_world_run_result runtime_world::run_presentation(jobs::job_system& jobs, const simulation_tick& tick,
                                                         float frame_delta_seconds, float interpolation_alpha,
                                                         runtime_service_provider* services,
                                                         const simulation_input_snapshot* input,
                                                         memory::tick_arena& tick_memory,
                                                         memory::frame_arena& frame_memory, std::uint64_t process_seed)
{
    if (state_ != runtime_world_state::running && state_ != runtime_world_state::paused) return {};
    if (!descriptor_.presentation_enabled) return {};
...
runtime_world_manager::runtime_world_manager(memory::memory_system& memory) : memory_(&memory) {}

runtime_world& runtime_world_manager::create(runtime_world_descriptor descriptor)
{
    if (executing_) throw std::logic_error("runtime worlds may only be created at frame boundaries");
...
bool runtime_world_manager::destroy(runtime_world_id id)
{
    ...
    const auto found =
        std::find_if(worlds_.begin(), worlds_.end(), [id](const auto& value) { return value->id() == id; });
...
void runtime_world_manager::rebuild_execution_order()
{
    ...
    std::sort(execution_order_.begin(), execution_order_.end(),
...
runtime_world_run_result runtime_world_manager::run_fixed(jobs::job_system& jobs, const simulation_tick& tick,
                                                          runtime_service_provider* services,
                                                          const simulation_input_snapshot* input,
                                                          memory::tick_arena& tick_memory,
                                                          memory::frame_arena& frame_memory, std::uint64_t process_seed)
{
    ...
    {
        runtime_world_run_result result =
            world->run_fixed(jobs, tick, services, input, tick_memory, frame_memory, process_seed);
        aggregate.systems_executed += result.systems_executed;
        aggregate.errors.insert(aggregate.errors.end(), std::make_move_iterator(result.errors.begin()),
                                std::make_move_iterator(result.errors.end()));
    }
    return aggregate;
}

runtime_world_run_result
runtime_world_manager::run_presentation(jobs::job_system& jobs, const simulation_tick& tick, float frame_delta_seconds,
                                        float interpolation_alpha, runtime_service_provider* services,
                                        const simulation_input_snapshot* input, memory::tick_arena& tick_memory,
                                        memory::frame_arena& frame_memory, std::uint64_t process_seed)
{
    runtime_world_run_result aggregate;
    execution_scope scope(executing_);
    ...
    {
        runtime_world_run_result result =
            world->run_presentation(jobs, tick, frame_delta_seconds, interpolation_alpha, services, input, tick_memory,
                                    frame_memory, process_seed);
        aggregate.systems_executed += result.systems_executed;
        aggregate.errors.insert(aggregate.errors.end(), std::make_move_iterator(result.errors.begin()),
                                std::make_move_iterator(result.errors.end()));
    }
...
world_snapshot_result runtime_world_manager::capture_snapshot(runtime_world_id world_id, simulation_tick_id tick,
                                                              double simulation_time_seconds, std::string label,
                                                              runtime_service_registry* services)
{
    if (executing_) return {false, {}, "snapshots may only be captured at a phase boundary"};
    if (snapshot_budget_bytes_ == 0) return {false, {}, "runtime snapshots are disabled"};
    runtime_world* world = find(world_id);
    if (!world) return {false, {}, "runtime world does not exist"};

    std::vector<runtime_service_snapshot> service_snapshots;
    if (services)
    {
        auto captured = services->capture_deterministic_state(world_id.value);
        if (!captured) return {false, {}, captured.error().message};
    ...
    world_snapshot_metadata metadata{
        ...
        .estimated_bytes = std::max<std::size_t>(estimated_entity_snapshot_bytes,
                                                 world->entities_->live_count() * estimated_entity_snapshot_bytes) +
                           service_bytes,
    ...
    stored_snapshot snapshot{.metadata = metadata,
...
world_snapshot_result runtime_world_manager::restore_snapshot(world_snapshot_id id, runtime_service_registry* services)
{
    if (executing_) return {false, {}, "snapshots may only be restored at a phase boundary"};
    const auto found = std::find_if(snapshots_.begin(), snapshots_.end(),
                                    [id](const stored_snapshot& value) { return value.metadata.id == id; });
    if (found == snapshots_.end()) return {false, {}, "snapshot does not exist"};

    runtime_world* world = find(found->metadata.world);
    if (!world) return {false, found->metadata, "snapshot target world does not exist"};

    if (!found->services.empty() && !services)
        return {false, found->metadata, "snapshot deterministic runtime services are unavailable"};
    if (services)
    {
        auto validated = services->validate_deterministic_state(world->id_.value, found->services);
        if (!validated) return {false, found->metadata, validated.error().message};
    }
    ...
    try
    {
        ...
        if (services) services->restore_deterministic_state(world->id_.value, found->services);
        ++world->epoch_;
        world->fault_message_.clear();
        if (world->state_ == runtime_world_state::faulted) world->state_ = runtime_world_state::paused;
