...
runtime::runtime(application& app) : runtime(app, app.configure()) {}

runtime::runtime(application& app, application_config config)
    : app_(&app), config_(normalize_config(std::move(config))), jobs_({.memory = &memory_}),
      module_context_(jobs_, diagnostics::default_logger(), memory_, memory::default_tracked_memory_resource(),
                      &services_, &worlds_)
{
...
{
    if (config.title.empty()) config.title = "ARC Application";
    if (config.initial_width == 0) config.initial_width = 1280;
    if (config.initial_height == 0) config.initial_height = 720;
    if (!valid_simulation_config(config.simulation)) config.simulation = simulation_config{};
    if (config.simulation.headless)
    {
        config.visible = false;
        config.start_focused = false;
        if (!config.simulation.allow_headless_time_controls) config.simulation.time_scale = 1.0;
    }
    return config;
}
    ...
    try
    {
        ...
        if (!worlds_registered_)
        {
            ...
            if (worlds_.size() == 0)
            {
                ...
                descriptor.name = descriptor.role == runtime_world_role::server           ? "Server World"
                                  : descriptor.role == runtime_world_role::editor_preview ? "Editor Preview World"
                                                                                          : "Client World";
...
{
    if (!started_) start();
    if (!running_) return current_time_;
...
frame_time runtime::advance(double wall_delta_seconds)
{
    ...
    if (!std::isfinite(wall_delta_seconds) || wall_delta_seconds < 0.0) wall_delta_seconds = 0.0;
    ...
    {
        sampled_input_.insert(sampled_input_.end(), pending_input_.begin(), pending_input_.end());
        pending_input_.clear();
        ++input_revision_;
    }
    const simulation_input_snapshot input_snapshot{.revision = input_revision_, .commands = sampled_input_};

    if (!paused_) accumulator_seconds_ += clamped_delta * config_.simulation.time_scale;
    ...
    const auto execute_tick = [&]()
    {
        ...
        current_tick_.total_seconds = static_cast<double>(current_tick_.id.value) * fixed_delta;
        const runtime_world_run_result result =
            worlds_.run_fixed(jobs_, current_tick_, &services_, &input_snapshot, tick_arena_, frame_arena_,
                              config_.simulation.process_seed);
        tick_arena_.reset();
        ++current_time_.completed_ticks;
        if (!result.succeeded() && config_.simulation.headless) request_stop();
    };
    ...
    else if (!paused_)
    {
        ...
        while (accumulator_seconds_ + 1e-12 >= fixed_delta &&
               catch_up_ticks < config_.simulation.maximum_catch_up_ticks && running_)
        {
    ...
    current_time_.interpolation_alpha =
        fixed_delta > 0.0 ? std::clamp(accumulator_seconds_ / fixed_delta, 0.0, 1.0) : 0.0;
    if (config_.simulation.presentation_enabled)
    {
        (void)worlds_.run_presentation(jobs_, current_tick_, static_cast<float>(clamped_delta),
                                       static_cast<float>(current_time_.interpolation_alpha), &services_,
                                       &input_snapshot, tick_arena_, frame_arena_, config_.simulation.process_seed);
    }
    modules_.update(module_context_, current_time_);
    app_->on_update(current_time_);
    jobs_.pump_main_thread();
    frame_arena_.reset();
    if (current_time_.completed_ticks != 0) sampled_input_.clear();
...
void runtime::dispatch(const event& value)
{
    ...
    if (is_input) pending_input_.push_back(input);
    if (value.type == event_type::close_requested) request_stop();
}
...
{
    if (!started_ || (config_.simulation.headless && !config_.simulation.allow_headless_time_controls)) return;
    paused_ = true;
    worlds_.pause_all();
}
...
bool runtime::step(std::uint32_t ticks) noexcept
{
    if (!started_ || !paused_ || ticks == 0 ||
        ...
        return false;
    pending_steps_ =
        std::min<std::uint32_t>(std::numeric_limits<std::uint32_t>::max() - pending_steps_, ticks) + pending_steps_;
    return true;
}
...
{
    return worlds_.capture_snapshot(world, current_tick_.id, current_tick_.total_seconds, std::move(label), &services_);
}
