                                          ...
                                          const std::filesystem::path& output)
{
    ...
    for (auto& [group, indices] : groups)
    {
        ...
        std::vector<std::byte> package(reinterpret_cast<const std::byte*>(package_magic.data()),
        ...
        if (!existing || hash_bytes(*existing) != package_hash)
        {
            if (existing)
            {
                ...
                if (filesystem_error)
                {
                    result.error = "Could not quarantine corrupt package chunk: " + filesystem_error.message();
                    return result;
                }
            }
            ...
            stream.write(reinterpret_cast<const char*>(package.data()), static_cast<std::streamsize>(package.size()));
            stream.close();
...
asset_package_mount::asset_package_mount() : implementation_(std::make_unique<implementation>()) {}
...
asset_status asset_package_mount::mount(const std::filesystem::path& manifest_path)
{
    ...
    for (const auto& artifact : staged.artifacts)
    {
        ...
        std::error_code filesystem_error;
        if (!std::filesystem::exists(package, filesystem_error) || artifact.stored_size == 0 || artifact.compressed)
        {
            return asset_status::failure(
                {.code = asset_error_code::invalid_metadata,
                 .guid = artifact.asset,
                 .path = package,
                 .message = artifact.compressed ? "This runtime does not support compressed ARC package records"
...
core::result<std::vector<std::byte>, asset_error> asset_package_mount::read(asset_guid asset,
                                                                            artifact_schema_id schema) const
{
    const auto found =
        std::find_if(implementation_->manifest.artifacts.begin(), implementation_->manifest.artifacts.end(),
...
asset_package_mount::read_async(asset_guid asset, artifact_schema_id schema, io::async_file_service& files,
                                jobs::cancellation_token cancellation) const
{
    ...
    if (found == implementation_->manifest.artifacts.end())
    {
        return files.scheduler().submit_future(
            {.name = "assets.package.missing", .affinity = jobs::job_affinity::io_thread, .cancellation = cancellation},
            []
            {
