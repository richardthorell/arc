...
bool write_atomic(const std::filesystem::path& destination, std::span<const std::byte> bytes, std::string& error)
{
    ...
    if (std::filesystem::exists(destination, filesystem_error) && !filesystem_error) return true;
...
std::optional<cache_action> parse_action(const std::filesystem::path& path, asset_hash expected)
{
    ...
    if (!document.is_object() || document.value("format", "") != "arc.ddc-action" || document.value("version", 0) != 1)
        return std::nullopt;
    const auto key = parse_asset_hash(document.value("key", ""));
    if (!key || *key != expected || !document.contains("artifacts") || !document["artifacts"].is_array())
        return std::nullopt;
    cache_action result;
    result.key = *key;
...
{
    return id_string(value.high, value.low);
}
...
std::string canonical_cook_target(const cook_target& target)
{
    ...
    std::ostringstream stream;
    stream << target.name << '|' << static_cast<unsigned>(target.platform) << '|'
           << static_cast<unsigned>(target.architecture) << '|' << static_cast<unsigned>(target.renderer) << '|'
           << static_cast<unsigned>(target.textures) << '|' << static_cast<unsigned>(target.configuration) << '|'
           << target.api_major << '.' << target.api_minor << '|' << (target.little_endian ? "le" : "be");
...
std::optional<std::vector<std::byte>> filesystem_shared_cache::get_blob(content_hash hash, cache_error& error)
{
...
bool filesystem_shared_cache::put_blob(content_hash hash, std::span<const std::byte> bytes, cache_error& error)
{
    ...
    {
        if (hash_bytes(*existing) == hash) return true;
...
std::optional<cache_action> filesystem_shared_cache::get_action(asset_build_key key, cache_error& error)
{
    std::lock_guard lock(implementation_->mutex);
...
bool filesystem_shared_cache::put_action(const cache_action& action, cache_error& error)
{
    ...
    {
        const std::string existing_text(reinterpret_cast<const char*>(existing->data()), existing->size());
        if (existing_text == serialized) return true;
...
struct http_shared_cache::implementation
{
    ...
    http_cache_request request(http_cache_method method, std::string path) const
    {
        ...
        if (!config.bearer_token.empty()) result.headers.emplace_back("authorization", "Bearer " + config.bearer_token);
        return result;
    }
};
...
std::optional<std::vector<std::byte>> http_shared_cache::get_blob(content_hash hash, cache_error& error)
{
    ...
    const auto response = implementation_->config.transport(
        implementation_->request(http_cache_method::get, "v1/blobs/sha256/" + hash_text));
...
bool http_shared_cache::put_blob(content_hash hash, std::span<const std::byte> bytes, cache_error& error)
{
    ...
    auto request = implementation_->request(http_cache_method::put, "v1/blobs/sha256/" + to_string(hash));
    ...
    if (response.status == 409)
    {
        ...
        if (existing && *existing == std::vector<std::byte>(bytes.begin(), bytes.end())) return true;
        error.message =
            verify_error ? verify_error.message : "HTTP shared cache rejected an immutable blob with different content";
        return false;
    }
    error.message = !response.error.empty()
                        ? response.error
...
std::optional<cache_action> http_shared_cache::get_action(asset_build_key key, cache_error& error)
{
    if (!implementation_->config.transport)
    {
        error.message = "HTTP shared cache has no transport";
    ...
    const auto response = implementation_->config.transport(
        implementation_->request(http_cache_method::get, "v1/actions/" + to_string(key)));
    if (response.status == 404) return std::nullopt;
    if (response.status != 200)
    {
        error.message = !response.error.empty()
    ...
    if (!result) error.message = "HTTP shared cache returned an invalid action";
    return result;
}
...
bool http_shared_cache::put_action(const cache_action& action, cache_error& error)
{
    ...
    const auto serialized = action_json(action).dump();
    auto request = implementation_->request(http_cache_method::put, "v1/actions/" + to_string(action.key));
    request.headers.emplace_back("if-none-match", "*");
    request.headers.emplace_back("content-type", "application/json");
    request.body.assign(reinterpret_cast<const std::byte*>(serialized.data()),
    ...
    if (response.status == 409)
    {
        ...
        if (existing && existing->artifacts == action.artifacts && existing->metadata == action.metadata) return true;
...
struct derived_data_cache::implementation
{
    ...
    {
        if (config.access == cache_access::read_only) return;
        const auto serialized = json{{"format", "arc.ddc-statistics"},
                                     {"version", 1},
        ...
        if (stream) stream.write(serialized.data(), static_cast<std::streamsize>(serialized.size()));
    }
    ...
    void refresh_size()
    {
        ...
        if (std::filesystem::exists(root, error))
            for (std::filesystem::recursive_directory_iterator it(root, error), end; it != end && !error;
                 it.increment(error))
                if (it->is_regular_file(error)) size += it->file_size(error);
...
std::optional<cache_blob> derived_data_cache::get_blob(content_hash hash, cache_error& error)
{
    ...
    if (implementation_->config.access != cache_access::offline && implementation_->config.shared)
    {
        ...
        if (implementation_->config.require_shared)
            error.message = shared_error ? std::move(shared_error.message) : "Required shared cache blob was not found";
    }
    return std::nullopt;
}
...
std::optional<cache_action> derived_data_cache::get_action(asset_build_key key, cache_error& error)
{
    ...
    if (implementation_->config.access != cache_access::offline && implementation_->config.shared)
    {
        ...
        if (implementation_->config.require_shared)
            error.message =
                shared_error ? std::move(shared_error.message) : "Required shared cache action was not found";
    }
    return std::nullopt;
}

bool derived_data_cache::put_blob(content_hash hash, std::span<const std::byte> bytes, cache_error& error)
{
    if (hash_bytes(bytes) != hash)
    {
    ...
    if (!write_atomic(destination, bytes, error.message)) return false;
    if (!existed)
    {
...
bool derived_data_cache::put_action(const cache_action& action, cache_error& error)
{
    ...
    if (!write_atomic(destination, std::as_bytes(std::span(serialized.data(), serialized.size())), error.message))
        return false;
    if (implementation_->config.access != cache_access::offline && implementation_->config.shared)
    {
        cache_error ignored;
        if (!implementation_->config.shared->put_action(action, ignored) && implementation_->config.require_shared)
        {
...
std::size_t derived_data_cache::verify(std::vector<std::string>* diagnostics)
{
    ...
    {
        if (!it->is_regular_file(error) || it->path().extension() == ".corrupt") continue;
        ...
        {
            ++implementation_->statistics.corrupt_entries;
            if (diagnostics) diagnostics->push_back("Corrupt cache blob: " + it->path().generic_string());
        }
    }
    return valid;
}
...
std::uint64_t derived_data_cache::prune(bool force)
{
    ...
    if (!force && implementation_->statistics.local_bytes <=
        ...
        return 0;
    const auto target = force ? 0ull : static_cast<std::uint64_t>(policy.maximum_bytes * policy.prune_target);
    struct candidate
    {
    ...
    if (std::filesystem::exists(root, error))
        for (std::filesystem::recursive_directory_iterator it(root, error), end; it != end && !error;
             it.increment(error))
            if (it->is_regular_file(error) && !implementation_->pins.contains(it->path().filename().string()))
    ...
    {
        if (implementation_->statistics.local_bytes <= target) break;
    ...
    const auto remove_expired = [&](const std::filesystem::path& root, auto lifetime, bool temporary_only)
    {
             ...
             it.increment(error))
        {
            ...
            if (temporary_only && it->path().filename().string().find(".tmp-") == std::string::npos) continue;
            if (now - it->last_write_time(error) > lifetime) std::filesystem::remove(it->path(), error);
            error.clear();
        }
    };
    remove_expired(implementation_->config.root, implementation_->config.cleanup.temporary_lifetime, true);
    remove_expired(implementation_->config.root / "actions", implementation_->config.cleanup.action_lifetime, false);
    return removed;
}
