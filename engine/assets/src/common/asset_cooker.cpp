...
std::string renderer_name(cook_renderer value)
{
    switch (value)
    {
    case cook_renderer::vulkan: return "vulkan";
    case cook_renderer::direct3d12: return "direct3d12";
    case cook_renderer::metal: return "metal";
    }
    return "vulkan";
}
...
std::string texture_family_name(cook_texture_family value)
{
    switch (value)
    {
    case cook_texture_family::bc: return "bc";
    case cook_texture_family::astc: return "astc";
    case cook_texture_family::etc2: return "etc2";
    case cook_texture_family::portable: return "portable";
    }
    return "bc";
}
...
bool parse_target(const json& value, cook_target& target)
{
    ...
    target.renderer = renderer == "direct3d12" ? cook_renderer::direct3d12 :
        renderer == "metal" ? cook_renderer::metal : cook_renderer::vulkan;
    ...
    target.textures = textures == "astc" ? cook_texture_family::astc :
        ...
        textures == "portable" ? cook_texture_family::portable : cook_texture_family::bc;
    target.configuration = value.value("configuration", "shipping") == "development"
        ? cook_configuration::development : cook_configuration::shipping;
...
std::optional<cached_artifact_metadata> parse_cached_artifact(const json& value)
{
    ...
    if (!high || !low || !hash)
        return std::nullopt;
    return cached_artifact_metadata{
...
struct asset_cooker::implementation
{
    ...
    asset_cook_processor* processor_for(asset_type_id type) const
    {
        for (const auto& processor : processors)
            if (std::find(processor->descriptor().input_types.begin(),
                    processor->descriptor().input_types.end(), type) !=
                processor->descriptor().input_types.end())
...
{
    if (!processor || !processor->descriptor().id.valid() ||
        !processor->descriptor().schema.valid() ||
        processor->descriptor().input_types.empty())
        return false;
    for (const auto& type : processor->descriptor().input_types)
        if (implementation_->processor_for(type))
            return false;
...
cook_result asset_cooker::cook(const cook_request& request)
{
    ...
    std::function<bool(asset_guid)> visit = [&](asset_guid guid) {
        ...
        {
            result.error = { .code = asset_error_code::dependency_cycle, .guid = guid,
        ...
        if (!asset)
        {
            result.error = { .code = asset_error_code::not_found, .guid = guid,
                .message = "Asset cook dependency is missing" };
            return false;
        }
        if (asset->state != asset_state::ready)
        {
            const auto imported = implementation_->assets->load<source_asset_data>({
    ...
    for (const auto guid : order)
    {
        ...
        {
            result.error = { .code = asset_error_code::importer_missing, .guid = guid,
        ...
        const auto key = make_asset_build_key({
        ...
        if (const auto action = implementation_->cache->get_action(key, cache_error_value))
        {
            ...
            if (metadata.is_array() && metadata.size() == action->artifacts.size())
            {
                ...
                if (complete)
                {
                    result.manifest.artifacts.insert(
                        result.manifest.artifacts.end(), cached.begin(), cached.end());
                    ++result.cache_hits;
        ...
        const auto loaded = implementation_->assets->load<source_asset_data>({
        ...
        result.diagnostics.insert(result.diagnostics.end(),
            cooked.diagnostics.begin(), cooked.diagnostics.end());
        if (!cooked.succeeded())
        {
            result.error = cooked.error ? cooked.error : asset_error{
        ...
        if (request.fail_on_warning && std::any_of(cooked.diagnostics.begin(), cooked.diagnostics.end(),
            [](const auto& diagnostic) { return diagnostic.severity != asset_diagnostic_severity::information; }))
        {
            result.error = { .code = asset_error_code::import_failed, .guid = guid,
    ...
    std::string& error)
{
    ...
    for (const auto& artifact : manifest.artifacts)
        artifacts.push_back({
            { "asset", to_string(artifact.asset) },
    ...
    for (const auto guid : manifest.roots) roots.push_back(to_string(guid));
    json closure = json::array();
    for (const auto guid : manifest.dependency_closure) closure.push_back(to_string(guid));
    const json document{
        { "format", "arc.cook-manifest" },
        { "version", manifest.version },
        { "buildId", manifest.build_id },
        { "target", target_json(manifest.target) },
        { "roots", std::move(roots) },
        { "dependencyClosure", std::move(closure) },
    ...
    std::string& error)
{
    ...
    for (const auto& value : document["artifacts"])
    {
        ...
        const auto hash = parse_asset_hash(value.value("hash", ""));
        const auto schema_value = parse_cached_artifact(json{
            { "name", value.value("name", "") },
            { "extension", "" },
