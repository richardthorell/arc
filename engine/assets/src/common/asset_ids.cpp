...
asset_hash_result hash_file(const std::filesystem::path& path)
{
    ...
    if (!input)
        return asset_hash_result::failure(
            {.code = asset_error_code::io_failed, .path = path, .message = "Could not open source file for hashing"});
    sha256 hash;
    ...
    if (!input.eof())
        return asset_hash_result::failure(
            {.code = asset_error_code::io_failed, .path = path, .message = "Source file hashing failed"});
