...
bool path_within(const std::filesystem::path& root, const std::filesystem::path& candidate)
{
    ...
    for (; root_iterator != normalized_root.end(); ++root_iterator, ++candidate_iterator)
        if (candidate_iterator == normalized_candidate.end() || path_key(*root_iterator) != path_key(*candidate_iterator))
            return false;
    return true;
}

std::optional<asset_guid> authored_asset_guid(
    const std::filesystem::path& source,
    asset_type_id type)
{
    if (type != asset_types::prefab)
        return std::nullopt;
    ...
    if (!document.is_object() || document.value("format", "") != "arc.prefab" ||
        ...
        !document["prefab"].contains("id") || !document["prefab"]["id"].is_string())
        return std::nullopt;
    return parse_asset_guid(document["prefab"]["id"].get<std::string>());
}

bool publish_artifact(
...
bool execute(sqlite3* database, const char* sql, std::string* error = nullptr)
{
    ...
    if (result == SQLITE_OK)
        return true;
    if (error)
        *error = sqlite_error ? sqlite_error : sqlite3_errmsg(database);
...
asset_reference dependency_from_path(
    const asset_import_context& context,
    std::string_view text,
    asset_type_id expected_type = {})
{
    if (text.empty() || text.starts_with("data:") || text.find("://") != std::string_view::npos)
        return {};
    std::filesystem::path authored(text);
    std::filesystem::path resolved;
    if (authored.is_absolute() || authored.has_root_name())
        return {};
    ...
    if (!expected_type.valid())
        if (const auto classification = classify_asset_path(relative))
            expected_type = classification->first;
...
void append_dependency(std::vector<asset_reference>& output, asset_reference reference)
{
    if (!reference.guid.valid() && reference.path_hint.empty())
        return;
    if (std::none_of(output.begin(), output.end(), [&](const auto& value) {
            return reference.guid.valid() ? value.guid == reference.guid :
                normalize_asset_path(value.path_hint) == normalize_asset_path(reference.path_hint);
        }))
        output.push_back(std::move(reference));
}

void collect_json_dependencies(
    const nlohmann::json& value,
    const asset_import_context& context,
    std::vector<asset_reference>& output,
    std::string_view parent_key = {})
{
    if (value.is_object())
    {
            ...
            value.contains("pathHint") && value["pathHint"].is_string())
        {
            ...
            if (value.contains("expectedType") && value["expectedType"].is_string())
                reference.expected_type = parse_asset_type_id(
                    value["expectedType"].get<std::string>()).value_or(asset_type_id{});
            const auto path_reference = dependency_from_path(
                context, value["pathHint"].get<std::string>(), reference.expected_type);
            reference.path_hint = path_reference.path_hint;
            if (!reference.expected_type.valid())
                reference.expected_type = path_reference.expected_type;
    ...
    const bool likely_asset_path =
        parent_key == "uri" || parent_key == "path" || parent_key == "material" ||
        parent_key == "prefabPath" || parent_key == "pathHint" ||
        classify_asset_path(std::filesystem::path(text)).has_value();
    if (likely_asset_path)
        append_dependency(output, dependency_from_path(context, text));
}

void collect_shader_dependencies(
    const asset_import_context& context,
    std::vector<asset_reference>& output)
{
    ...
    while ((cursor = source.find("#include", cursor)) != std::string::npos)
    {
        ...
        if (end == std::string::npos)
            break;
        append_dependency(output, dependency_from_path(
            context,
            std::string_view(source).substr(quote + 1, end - quote - 1),
            asset_types::shader));
        cursor = end + 1;
    }
}
...
{
public:
    source_blob_importer(asset_importer_id id, asset_type_id type, std::string name, std::vector<std::string> extensions)
    {
        descriptor_.id = id;
    ...
    const asset_importer_descriptor& descriptor() const noexcept override { return descriptor_; }
    ...
    asset_import_result import(const asset_import_context& context) override
    {
        ...
        {
            return { .error = {
                .code = asset_error_code::cancelled,
        ...
        result.payload = asset_payload::make<source_asset_data>(
        ...
        if (context.metadata.type == asset_types::scene ||
            context.metadata.type == asset_types::prefab ||
            context.metadata.type == asset_types::material ||
            context.source_path.extension() == ".gltf")
        {
            const auto document = nlohmann::json::parse(
                reinterpret_cast<const char*>(context.source_bytes.data()),
                reinterpret_cast<const char*>(context.source_bytes.data()) + context.source_bytes.size(),
                nullptr,
                false);
            if (!document.is_discarded())
                collect_json_dependencies(document, context, result.dependencies);
        }
...
std::vector<std::unique_ptr<asset_importer>> default_importers()
{
    ...
    result.push_back(std::make_unique<source_blob_importer>(
        importer_ids::scene, asset_types::scene, "ARC Scene", std::vector<std::string>{ ".arcscene" }));
    result.push_back(std::make_unique<source_blob_importer>(
        importer_ids::prefab, asset_types::prefab, "ARC Prefab", std::vector<std::string>{ ".arcprefab" }));
    result.push_back(std::make_unique<source_blob_importer>(
        importer_ids::material, asset_types::material, "ARC Material", std::vector<std::string>{ ".arcmat" }));
    result.push_back(std::make_unique<source_blob_importer>(
    ...
    result.push_back(std::make_unique<source_blob_importer>(
        importer_ids::environment, asset_types::environment, "Environment",
        std::vector<std::string>{ ".hdr", ".exr" }));
    result.push_back(std::make_unique<source_blob_importer>(
        importer_ids::gltf, asset_types::imported_scene, "glTF",
...
struct asset_manager::implementation
{
        ...
        , memory(&memory_system)
    {
        ...
        if (config.asset_root.empty())
            config.asset_root = config.project_root / "assets";
        if (config.cache_root.empty())
            config.cache_root = config.project_root / ".arc" / "cache";
    ...
    {
        if (pressure_handler)
            memory->remove_pressure_handler(pressure_handler);
        if (database)
    ...
    void emit(asset_event_kind kind, asset_guid guid, asset_state state, std::string message, float progress = 0.0f)
    {
        ...
        if (events.size() > 4096)
            events.erase(events.begin(), events.begin() + 1024);
    ...
    asset_diagnostic diagnostic(
        asset_guid guid,
        asset_diagnostic_severity severity,
        std::string category,
        std::string message)
    {
        return {
            .sequence = next_diagnostic++,
            .severity = severity,
            .guid = guid,
    ...
    bool open_database(std::string& error)
    {
        ...
        execute(database, "PRAGMA foreign_keys=ON;");
        {
            ...
            if (!integrity || sqlite3_step(integrity.get()) != SQLITE_ROW ||
        ...
        {
            sqlite_statement version(database,
    ...
    bool rebuild_database(std::string& error)
    {
        ...
        const auto corrupt_path = config.cache_root /
            ("assets.db.corrupt-" + std::to_string(
    ...
    bool load_database(std::string& error)
    {
        ...
        sqlite_statement dependencies_statement(database,
            "SELECT asset_guid,dependency_guid FROM dependencies ORDER BY asset_guid,dependency_guid;");
        ...
        while (diagnostics_statement && sqlite3_step(diagnostics_statement.get()) == SQLITE_ROW)
        {
            ...
            if (const auto found = records.find(*owner); found != records.end())
            {
                const auto sequence = static_cast<std::uint64_t>(
                    sqlite3_column_int64(diagnostics_statement.get(), 1));
    ...
    {
        if (!database || value.virtual_asset)
            return true;
        sqlite_statement statement(database,
            "INSERT INTO assets(guid,type,importer,source_path,source_hash,dependency_hash,state,"
        ...
        sqlite_statement insert_diagnostic(database,
            "INSERT INTO diagnostics(asset_guid,sequence,severity,category,message) VALUES(?,?,?,?,?);");
        for (const auto& value : snapshot.diagnostics)
        {
            ...
            sqlite3_bind_int64(insert_diagnostic.get(), 2, static_cast<sqlite3_int64>(value.sequence));
            sqlite3_bind_int(insert_diagnostic.get(), 3, static_cast<int>(value.severity));
            bind_text(insert_diagnostic.get(), 4, value.category);
            bind_text(insert_diagnostic.get(), 5, value.message);
    ...
    void persist_artifacts(const record& value)
    {
        ...
        sqlite3_step(remove.get());
        sqlite_statement insert(database,
            "INSERT INTO artifacts(asset_guid,name,path,content_hash,size,residency) VALUES(?,?,?,?,?,?);");
    ...
    bool dependency_reaches(asset_guid current, asset_guid target, std::unordered_set<asset_guid, asset_guid_hash>& visited) const
    {
        if (current == target)
            return true;
        ...
        if (found == records.end())
            return false;
        return std::any_of(
            found->second.snapshot.dependencies.begin(),
            found->second.snapshot.dependencies.end(),
    ...
    void mark_reverse_stale(asset_guid guid, std::string_view reason)
    {
        ...
        for (asset_guid dependent : found->second.snapshot.reverse_dependencies)
        {
            ...
            if (dependent_found == records.end() ||
                dependent_found->second.snapshot.state == asset_state::stale)
                continue;
            dependent_found->second.snapshot.state = asset_state::stale;
            dependent_found->second.snapshot.revision = ++revision;
            dependent_found->second.snapshot.diagnostics.push_back(diagnostic(
                dependent,
                asset_diagnostic_severity::information,
                "dependency",
        ...
        asset_residency requested_residency = asset_residency::cpu)
    {
        ...
        if (importer_found == importers.end())
        {
            ...
            value.snapshot.diagnostics.push_back(diagnostic(
                guid, asset_diagnostic_severity::error, "import", "No importer is registered for this asset"));
        ...
        value.active_import = jobs->submit({
                ...
                if (dependency_job.valid())
                {
                    ...
                    if (!result.succeeded())
                    {
                        ...
                        if (const auto current = records.find(guid); current != records.end())
                        {
                            ...
                            current->second.snapshot.revision = ++revision;
                            current->second.snapshot.diagnostics.push_back(diagnostic(
                                guid, asset_diagnostic_severity::error, "dependency", "An asset dependency failed to import"));
                            persist_record(current->second);
            ...
            if (!read)
            {
                ...
                if (const auto current = records.find(guid); current != records.end())
                {
                    ...
                    current->second.snapshot.revision = ++revision;
                    current->second.snapshot.diagnostics.push_back(diagnostic(
                        guid, asset_diagnostic_severity::error, "io", read.error().message));
                    persist_record(current->second);
            ...
            asset_importer* importer{};
            {
                ...
                if (importer_iterator != importers.end())
                    importer = importer_iterator->second.get();
            }
            if (!importer)
                return;
            ...
            asset_import_result imported = importer->import({
                .reference = { guid, metadata.type, normalize_asset_path(
                    std::filesystem::relative(source_path, config.project_root)) },
                .metadata = metadata,
            ...
            if (imported.succeeded() && !imported.dependencies.empty())
            {
                ...
                {
                    if (invalid_prerequisite || effective_cancellation.stop_requested())
                        break;
                    const auto dependency_job = ensure_import(
                        dependency_guid, priority, {});
                    if (dependency_job.valid() && !dependency_job.wait_result().succeeded())
                        invalid_prerequisite = true;
                }
                if (invalid_prerequisite)
                    imported.error = {
                        .code = asset_error_code::dependency_failed,
                        .guid = guid,
                        .path = source_path,
            ...
            if (!imported.succeeded())
            {
                ...
                target.snapshot.diagnostics.insert(
                    target.snapshot.diagnostics.end(),
                    imported.diagnostics.begin(),
                    imported.diagnostics.end());
                target.snapshot.diagnostics.push_back(diagnostic(
                    guid,
                    asset_diagnostic_severity::error,
                    "import",
                    imported.error.message.empty() ? "Asset import failed" : imported.error.message));
                persist_record(target);
                emit(asset_event_kind::failed, guid, asset_state::failed, "Asset import failed; retaining last-good generation");
                return;
            }
            ...
            if (invalid_dependency)
            {
                ...
                target.snapshot.revision = ++revision;
                target.snapshot.diagnostics.push_back(diagnostic(
                    guid, asset_diagnostic_severity::error, "dependency",
            ...
            if (imported.dependencies_authoritative)
            {
                ...
                for (const auto dependency_guid : resolved_dependencies)
                {
                    ...
                    if (std::find(reverse.begin(), reverse.end(), guid) == reverse.end())
                        reverse.push_back(guid);
                }
            }
            ...
            if (!imported.subassets.empty())
            {
                ...
                for (auto subasset : imported.subassets)
                {
                    const auto old = std::find_if(target.metadata.subassets.begin(), target.metadata.subassets.end(),
                        [&](const auto& value) { return value.persistent_key == subasset.persistent_key; });
                ...
                {
                    if (std::none_of(merged.begin(), merged.end(), [&](const auto& value) {
                            return value.persistent_key == old.persistent_key;
                        }))
                    {
                        old.tombstoned = true;
            ...
            std::vector<asset_hash> key_parts{
                ...
                hash_bytes(std::as_bytes(std::span(metadata.canonical_settings.data(), metadata.canonical_settings.size()))),
                ...
                hash_bytes(std::as_bytes(std::span(
                    importer_version_text.data(), importer_version_text.size()))),
            ...
            const auto directory = config.cache_root / "derived" / config.target_profile /
                key_text.substr(0, 2) / key_text;
            std::error_code filesystem_error;
            ...
            if (!filesystem_error)
            {
                for (const auto& artifact : imported.artifacts)
                {
                    ...
                    {
                        target.snapshot.artifacts.push_back({
                            .name = artifact.name,
                            .path = destination,
                        ...
                        target.snapshot.diagnostics.push_back(diagnostic(
                            guid, asset_diagnostic_severity::error, "derived-data", std::move(publish_error)));
                }
            }

            target.slot->payload.store(
                std::make_shared<const asset_payload>(std::move(imported.payload)),
            ...
            if (database)
            {
                sqlite_statement generation(database,
                    "INSERT OR REPLACE INTO import_generations("
                    "asset_guid,generation,dependency_hash,published,status) VALUES(?,?,?,?,?);");
                bind_text(generation.get(), 1, to_string(guid));
                sqlite3_bind_int64(generation.get(), 2,
                    static_cast<sqlite3_int64>(target.snapshot.generation));
                bind_text(generation.get(), 3, to_string(target.snapshot.dependency_hash));
                sqlite3_bind_int64(generation.get(), 4,
...
{
    if (slot_)
        slot_->pins.fetch_sub(1, std::memory_order_relaxed);
    slot_.reset();
}
...
void asset_manager::on_shutdown(framework::runtime_service_context&) noexcept
{
    std::vector<jobs::job_handle> active;
    {
        ...
        for (auto& [_, record] : implementation_->records)
        {
            ...
            if (record.active_import.valid())
                active.push_back(record.active_import);
        }
    }
...
bool asset_manager::register_virtual_asset(
    asset_guid guid,
    asset_type_id type,
    ...
    bool pin_value)
{
    if (!guid.valid() || !type.valid() || payload.type() != type || !payload)
        return false;
    std::unique_lock lock(implementation_->mutex);
    if (implementation_->records.contains(guid))
        return false;
...
bool asset_manager::set_fallback(asset_type_id type, asset_guid guid)
{
    ...
    if (!type.valid() || found == implementation_->records.end() || found->second.snapshot.type != type)
        return false;
...
asset_scan_result asset_manager::scan()
{
    ...
    for (const auto& root : implementation_->config.additional_source_roots)
        if (std::none_of(source_roots.begin(), source_roots.end(), [&](const auto& value) {
                return path_key(value) == path_key(root);
            }))
    ...
    for (const auto& source_root : source_roots)
    {
             ...
             iterator.increment(error))
        {
            ...
            if (!loaded_metadata)
            {
                ...
                if (!implementation_->config.create_missing_metadata)
                    continue;
                source.metadata.guid = authored_asset_guid(source.absolute, classification->first)
            ...
            if (!source.metadata_created && implementation_->config.enable_source_monitor)
            {
                ...
                {
                    if (existing->second.modified == source.modified &&
                        existing->second.file_size == source.size)
                    {
                    ...
                    else if (implementation_->config.change_debounce.count() > 0)
                    {
                        ...
                        const bool settled =
                            ...
                            now - existing->second.pending_since >=
                                implementation_->config.change_debounce;
                        if (!settled)
                        {
    ...
    for (auto& source : discovered)
    {
        if (source.metadata_created)
        {
            ...
            for (const auto& [candidate_guid, candidate] : implementation_->records)
            {
                ...
                if (!candidate.virtual_asset &&
                    candidate.snapshot.type == source.metadata.type &&
        ...
        if (source.hash_deferred)
        {
            ...
            {
                if (!pending->second.pending_source_change ||
        ...
        if (found == implementation_->records.end())
        {
            ...
            implementation_->emit(asset_event_kind::discovered, source.metadata.guid, asset_state::stale, "Asset discovered");
            ++result.discovered;
            continue;
        }

        implementation::record& value = found->second;
        const bool observed_file_change =
            value.modified != source.modified || value.file_size != source.size;
        ...
        {
            const auto now = std::chrono::steady_clock::now();
            if (!value.pending_source_change ||
        ...
        const bool settings_changed =
            value.metadata.importer != source.metadata.importer ||
        ...
        const bool importer_changed =
            registered_importer_version != 0 &&
            value.snapshot.imported_version != 0 &&
        ...
        if (path_changed)
        {
            ...
            implementation_->paths[path_key(relative_text)] = value.snapshot.guid;
            value.snapshot.source_path = relative_text;
            implementation_->emit(asset_event_kind::moved, value.snapshot.guid, value.snapshot.state, "Asset path changed");
        }
        value.metadata = source.metadata;
        value.absolute_path = source.absolute;
        ...
        if (source_changed || settings_changed || importer_changed)
        {
            ...
            implementation_->emit(asset_event_kind::state_changed, value.snapshot.guid, asset_state::stale, "Asset source changed");
    ...
    {
        if (value.virtual_asset || seen.contains(guid) || value.snapshot.source_missing)
            continue;
        value.snapshot.source_missing = true;
        value.snapshot.state = asset_state::unknown;
        value.snapshot.revision = ++implementation_->revision;
        value.snapshot.diagnostics.push_back(implementation_->diagnostic(
            guid, asset_diagnostic_severity::error, "missing", "Asset source file is missing"));
        implementation_->persist_record(value);
        if (implementation_->database)
        {
            sqlite_statement tombstone(implementation_->database,
            ...
            sqlite3_bind_int64(tombstone.get(), 4,
                static_cast<sqlite3_int64>(value.snapshot.revision));
    ...
    {
        result.diagnostics.push_back(implementation_->diagnostic(
            {}, asset_diagnostic_severity::warning, "scan", error.message()));
    }
    lock.unlock();
...
{
    if (!implementation_->config.enable_source_monitor)
        return;
    const auto now = std::chrono::steady_clock::now();
    {
        std::shared_lock lock(implementation_->mutex);
        if (!implementation_->started || now < implementation_->next_poll)
            return;
    }
    {
        std::unique_lock lock(implementation_->mutex);
...
std::optional<asset_snapshot> asset_manager::find(asset_guid guid) const
{
    ...
    result.strong_references = found->second.slot.use_count() > 0
        ? static_cast<std::uint32_t>(found->second.slot.use_count() - 1)
        : 0;
...
std::optional<asset_snapshot> asset_manager::find(std::string_view project_relative_path) const
{
    ...
    if (path == implementation_->paths.end())
        return std::nullopt;
...
std::vector<asset_snapshot> asset_manager::search(
    std::string_view text,
    std::optional<asset_type_id> type) const
{
    std::shared_lock lock(implementation_->mutex);
    ...
    {
        if (type && value.snapshot.type != *type)
            continue;
        if (!needle.empty() && path_key(value.snapshot.source_path).find(needle) == std::string::npos)
            continue;
        auto snapshot = value.snapshot;
        snapshot.strong_references = value.slot.use_count() > 0
            ? static_cast<std::uint32_t>(value.slot.use_count() - 1)
            : 0;
...
asset_reference asset_manager::resolve(
    ...
    asset_type_id expected_type) const
{
    ...
    if (!found || (expected_type.valid() && found->type != expected_type))
        return result;
    result.guid = found->guid;
    if (!result.expected_type.valid())
        result.expected_type = found->type;
    return result;
}

missing_asset_reference asset_manager::audit_reference(
    const asset_reference& reference,
    std::string owner,
    std::string field)
{
    missing_asset_reference result{
        .reference = reference,
        .owner = std::move(owner),
        .field = std::move(field)
    };
    const auto found = find(reference.guid);
    if (found && !found->source_missing &&
        (!reference.expected_type.valid() || reference.expected_type == found->type))
        return result;
    result.reason = !reference.guid.valid() ? "Reference has no asset GUID" :
        !found ? "Asset GUID is not registered" :
        found->source_missing ? "Asset source is missing" : "Asset type does not match the reference";
    if (!reference.path_hint.empty())
    {
        if (const auto candidate = find(reference.path_hint))
            result.repair_candidates.push_back(candidate->guid);
    }
    std::unique_lock lock(implementation_->mutex);
...
bool asset_manager::set_dependencies(
    asset_guid guid,
    std::span<const asset_reference> dependencies_value)
{
    std::unique_lock lock(implementation_->mutex);
    const auto found = implementation_->records.find(guid);
    if (found == implementation_->records.end())
    ...
    for (const asset_reference& reference : dependencies_value)
    {
        if (!reference.guid.valid())
            return false;
        std::unordered_set<asset_guid, asset_guid_hash> visited;
        if (implementation_->dependency_reaches(reference.guid, guid, visited))
            return false;
    ...
    for (asset_guid dependency_guid : dependencies)
    {
        if (const auto dependency = implementation_->records.find(dependency_guid);
            ...
            std::find(dependency->second.snapshot.reverse_dependencies.begin(),
                dependency->second.snapshot.reverse_dependencies.end(), guid) ==
    ...
    implementation_->emit(asset_event_kind::dependencies_changed, guid, asset_state::stale, "Asset dependencies changed");
    return true;
}
...
bool asset_manager::mark_stale(asset_guid guid, std::string reason)
{
    ...
    found->second.snapshot.state = asset_state::stale;
    found->second.snapshot.revision = ++implementation_->revision;
    found->second.snapshot.diagnostics.push_back(implementation_->diagnostic(
        guid, asset_diagnostic_severity::information, "stale", std::move(reason)));
    implementation_->persist_record(found->second);
...
bool asset_manager::cancel_import(asset_guid guid)
{
    std::shared_lock lock(implementation_->mutex);
    const auto found = implementation_->records.find(guid);
    return found != implementation_->records.end() &&
        found->second.import_cancellation.request_cancel();
}
...
asset_move_result asset_manager::move(asset_guid guid, std::filesystem::path destination)
{
    ...
    if (destination.is_relative())
        destination = implementation_->config.project_root / destination;
    ...
    {
        result.error = {
            .code = asset_error_code::invalid_request,
            .guid = guid,
            .path = destination,
            .message = "Asset destination must remain inside a managed project source root and preserve its type"
        };
        return result;
    }
    ...
    if (error)
    {
        result.error = { .code = asset_error_code::io_failed, .guid = guid, .path = destination, .message = error.message() };
        return result;
    }
...
asset_move_result asset_manager::rename(asset_guid guid, std::string filename)
{
    ...
    if (!found)
        return { .guid = guid, .error = {
            .code = asset_error_code::not_found, .guid = guid, .message = "Asset was not found" } };
...
jobs::job_future<asset_manager::untyped_load_result> asset_manager::load_untyped(asset_load_request request)
{
    ...
    if (immediate_slot || immediate_error)
    {
        return implementation_->jobs->submit_future({
            .name = "assets.load.immediate",
            .priority = to_job_priority(request.priority)
        }, [slot = std::move(immediate_slot), error = std::move(immediate_error),
    ...
    return implementation_->jobs->submit_future({
        ...
        const auto found = implementation_->records.find(guid);
        if (found != implementation_->records.end() &&
            found->second.slot->payload.load(std::memory_order_acquire))
        {
            asset_error error;
            if (found->second.snapshot.state == asset_state::failed)
            {
                error = {
                    .code = asset_error_code::import_failed,
                    .guid = guid,
        ...
        return untyped_load_result{
            .error = {
                .code = request.cancellation.stop_requested()
                    ? asset_error_code::cancelled
                    : asset_error_code::import_failed,
                .guid = guid,
                .message = request.cancellation.stop_requested()
...
std::size_t asset_manager::evict_unused(asset_residency maximum_residency)
{
    ...
    for (auto& [_, value] : implementation_->records)
    {
        if (value.virtual_asset || value.slot.use_count() != 1 ||
            value.slot->pins.load(std::memory_order_relaxed) != 0 ||
            value.snapshot.residency > maximum_residency ||
    ...
    for (auto* value : candidates)
    {
        ...
        value->snapshot.revision = ++implementation_->revision;
        implementation_->persist_record(*value);
        implementation_->emit(asset_event_kind::evicted, value->snapshot.guid, value->snapshot.state, "Unused asset payload evicted");
    }
...
std::vector<asset_event> asset_manager::events_since(std::uint64_t sequence) const
{
    ...
    std::copy_if(
        implementation_->events.begin(),
        implementation_->events.end(),
