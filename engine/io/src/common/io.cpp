...
std::filesystem::path temporary_path_for(const std::filesystem::path& destination)
{
    ...
    return destination.parent_path() / (destination.filename().string() + ".arc-tmp-" + std::to_string(stamp));
}

file_result<file_buffer> read_range_sync(const std::filesystem::path& path, std::uint64_t offset,
                                         std::optional<std::size_t> requested_bytes, std::size_t chunk_size,
                                         const jobs::cancellation_token& cancellation)
{
    std::ifstream stream(path, std::ios::binary | std::ios::ate);
    if (!stream) return file_result<file_buffer>::failure(open_error(path));

    const auto end = stream.tellg();
    if (end < 0)
        return file_result<file_buffer>::failure(
            make_error(file_error_code::read_failed, path, "File size could not be read"));
    const auto file_size = static_cast<std::uint64_t>(end);
    if (offset > file_size)
        return file_result<file_buffer>::failure(
            make_error(file_error_code::invalid_range, path, "Read offset is beyond the end of the file"));

    const auto available = file_size - offset;
    const auto desired = requested_bytes.has_value() ? static_cast<std::uint64_t>(*requested_bytes) : available;
    if (desired > available)
        return file_result<file_buffer>::failure(
            make_error(file_error_code::invalid_range, path, "Read range extends beyond the end of the file"));
    if (desired > static_cast<std::uint64_t>(std::numeric_limits<std::size_t>::max()))
        return file_result<file_buffer>::failure(
            make_error(file_error_code::invalid_range, path, "File is too large for this process"));
    ...
    while (completed < result.size())
    {
        if (cancellation.stop_requested())
            return file_result<file_buffer>::failure(
                make_error(file_error_code::cancelled, path, "Read was cancelled"));
        ...
        if (stream.gcount() != static_cast<std::streamsize>(count))
            return file_result<file_buffer>::failure(
                make_error(file_error_code::read_failed, path, "File read ended unexpectedly"));
        completed += count;
    }
    return file_result<file_buffer>::success(std::move(result));
}

file_result<void> write_sync(const std::filesystem::path& path, const file_buffer& bytes, std::size_t chunk_size,
                             const jobs::cancellation_token& cancellation)
{
    ...
    if (directory_error)
        return file_result<void>::failure(
            make_error(file_error_code::permission_denied, path, "Destination directory could not be created"));

    std::ofstream stream(path, std::ios::binary | std::ios::trunc);
    if (!stream)
        return file_result<void>::failure(
            make_error(file_error_code::permission_denied, path, "File could not be opened for writing"));
    std::size_t completed{};
...
async_file_service::async_file_service(jobs::job_system& jobs, async_file_config config) : jobs_(&jobs), config_(config)
{
    config_.chunk_size = std::max<std::size_t>(config_.chunk_size, 4096);
}

jobs::job_future<file_result<file_buffer>> async_file_service::read_all(std::filesystem::path path,
...
jobs::job_future<file_result<file_buffer>> async_file_service::read_range(std::filesystem::path path,
...
jobs::job_future<file_result<void>> async_file_service::write(std::filesystem::path path,
...
jobs::job_future<file_result<void>> async_file_service::write_atomic(std::filesystem::path path,
                                                                     std::span<const std::byte> bytes,
                                                                     jobs::cancellation_token cancellation)
{
    file_buffer owned(bytes.begin(), bytes.end());
    return jobs_->submit_future(
        ...
        [path = std::move(path), bytes = std::move(owned), cancellation, chunk = config_.chunk_size]
        {
            ...
            if (cancellation.stop_requested())
            {
                ...
                std::filesystem::remove(temporary, ignored);
                return file_result<void>::failure(
                    make_error(file_error_code::cancelled, path, "Atomic write was cancelled"));
            }
            if (!replace_file(temporary, path))
            {
                ...
                std::filesystem::remove(temporary, ignored);
                return file_result<void>::failure(
                    make_error(file_error_code::replace_failed, path, "Atomic destination replacement failed"));
            }
            return file_result<void>::success();
        });
}

jobs::job_future<file_result<file_info>> async_file_service::stat(std::filesystem::path path,
                                                                  jobs::cancellation_token cancellation)
{
    return jobs_->submit_future(
        {.name = "io.stat",
        ...
        {
            if (cancellation.stop_requested())
                return file_result<file_info>::failure(
                    make_error(file_error_code::cancelled, path, "Stat was cancelled"));
            std::error_code error;
            const bool regular = std::filesystem::is_regular_file(path, error);
            if (error || !regular) return file_result<file_info>::failure(open_error(path));
            ...
            if (error)
                return file_result<file_info>::failure(
                    make_error(file_error_code::read_failed, path, "Modification time could not be read"));
            return file_result<file_info>::success({.size = size, .modified = modified, .regular_file = true});
        });
}
